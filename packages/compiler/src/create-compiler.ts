/**
 * @module @enterstellar/compiler/create-compiler
...
function resolveConfig(config: CompilerConfigInput): CompilerConfig {
  ...
  if (config.onCorrection !== undefined && config.selfCorrection?.llm !== undefined) {
    console.warn(
      '[@enterstellar/compiler] DEPRECATION: Both `onCorrection` and `selfCorrection.llm` are set. ' +
  ...
  const resolved: CompilerConfig = {
    ...
    ...(config.onCorrection !== undefined
      ...
      : {}),
    ...(config.onTelemetry !== undefined ? { onTelemetry: config.onTelemetry } : {}),
    ...(config.selfCorrection !== undefined ? { selfCorrection: config.selfCorrection } : {}),
  };
  ...
  if (
    threshold !== undefined &&
    (threshold < MIN_ENUM_THRESHOLD || threshold > MAX_ENUM_THRESHOLD)
  ) {
    throw new EnterstellarError(
      'ENS-2001',
      'compiler',
...
export function createCompiler(configInput: CompilerConfigInput): EnterstellarCompiler {
  ...
  const cache = createCompilationCache(500, (event, handler) => config.registry.on(event, handler));

  return {
    async compile(intent: ComponentIntent, options?: CompileOptions): Promise<CompilationResult> {
      return compile(intent, config, customSteps, cache, options);
    },

    async lint(intent: ComponentIntent): Promise<LintResult> {
