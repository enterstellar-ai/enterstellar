/**
 * @module @enterstellar/compiler/pipeline/token-step
...
import { isTokenReference, tokenExists, findNearestToken } from '../utils/token-utils.js';
  ...
  for (const [fieldPath, expectedTokenRef] of Object.entries(contractTokens)) {
    ...
    if (!isTokenReference(actualValue)) {
      ...
      if (config.strictDesignTokens) {
        context.errors.push(
          invalidTokenError(`props.${fieldPath}`, displayValue, expectedTokenRef),
        );
      } else {
    ...
    if (!tokenExists(actualValue, designTokens)) {
      if (config.strictDesignTokens) {
        ...
        context.errors.push(invalidTokenError(`props.${fieldPath}`, actualValue, nearest));
      } else {
