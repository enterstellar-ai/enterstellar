/**
 * @module @enterstellar/compiler/__tests__/create-compiler
...
describe('createCompiler', () => {
  ...
  describe('config validation', () => {
    it('throws on maxNestingDepth below 3', () => {
      const registry = createMockRegistry();
      expect(() => createCompiler({ registry, maxNestingDepth: 2 })).toThrow();
    });

    it('throws on maxNestingDepth above 20', () => {
      const registry = createMockRegistry();
      expect(() => createCompiler({ registry, maxNestingDepth: 21 })).toThrow();
    });
    ...
    it('accepts maxNestingDepth at boundary values (3 and 20)', () => {
      const registry = createMockRegistry();
      expect(() => createCompiler({ registry, maxNestingDepth: 3 })).not.toThrow();
      expect(() => createCompiler({ registry, maxNestingDepth: 20 })).not.toThrow();
    });
  });
