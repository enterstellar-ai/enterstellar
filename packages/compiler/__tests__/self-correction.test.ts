/**
 * @module @enterstellar/compiler/__tests__/self-correction
...
function createMockConfig(overrides: Partial<CompilerConfig> = {}): CompilerConfig {
  return {
...
describe('executeSelfCorrection', () => {
  it('returns corrected=false when no callback is provided', async () => {
    ...
    const result = await executeSelfCorrection(mockErrors, mockIntent, {}, config);
  ...
  it('retries up to maxRetries on callback failure', async () => {
    const onCorrection = vi.fn().mockRejectedValue(new Error('fail'));
