/**
 * @module @enterstellar/compiler/__tests__/integration/compile-invalid
 * @description Integration test: compile 20 invalid intents against the example registry.
 *
 * Uses the 10 example clinical-domain components from `@enterstellar/registry`.
...
describe('Integration: compile 20 invalid intents', () => {
  ...
  it('PatientVitals — missing required patientId', async () => {
    const result = await compiler.compile(intent('PatientVitals', { riskLevel: 'high' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  ...
  it('MedicationList — wrong type: patientId as number', async () => {
    const result = await compiler.compile(intent('MedicationList', { patientId: 12345 }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  ...
  it('DiagnosisSummary — wrong type: patientId as boolean', async () => {
    const result = await compiler.compile(intent('DiagnosisSummary', { patientId: true }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  ...
  it('DiagnosisSummary — missing all required fields', async () => {
    const result = await compiler.compile(intent('DiagnosisSummary', {}), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  ...
  it('AlertBanner — missing all required fields', async () => {
    const result = await compiler.compile(intent('AlertBanner', {}), { agent: 'integration-test' });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  });
  ...
  it('ClinicalNote — missing all required fields', async () => {
    const result = await compiler.compile(intent('ClinicalNote', { noteType: 'progress' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.filter((e) => e.code === 'ENS-2001').length).toBeGreaterThanOrEqual(1);
  ...
  it('GenericCard — wrong type: title as number', async () => {
    const result = await compiler.compile(intent('GenericCard', { title: 42 }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
    expect(result.errors.some((e) => e.code === 'ENS-2001')).toBe(true);
  ...
  it('unknown component produces ENS-2004', async () => {
    const result = await compiler.compile(intent('HallucinatedWidget', { foo: 'bar' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
  ...
  it('all errors have code, path, and message', async () => {
    const result = await compiler.compile(intent('PatientVitals', {}), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('fail');
...
describe('Integration: self-correction with example registry', () => {
  ...
  it('fallback renders GenericCard when self-correction exhausted', async () => {
    ...
    const result = await fallbackCompiler.compile(intent('GenericCard', { title: false }), {
      agent: 'fallback-test',
    });
