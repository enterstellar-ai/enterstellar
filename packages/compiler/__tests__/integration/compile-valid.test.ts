/**
 * @module @enterstellar/compiler/__tests__/integration/compile-valid
...
describe('Integration: compile 20 valid intents', () => {
  ...
  it('DiagnosisSummary — minimal', async () => {
    const result = await compiler.compile(intent('DiagnosisSummary', { patientId: 'p-001' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('pass');
    expect(result.errors).toHaveLength(0);
  ...
  it('LabResults — minimal', async () => {
    const result = await compiler.compile(intent('LabResults', { patientId: 'p-001' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('pass');
    expect(result.errors).toHaveLength(0);
  ...
  it('GenericCard — minimal', async () => {
    const result = await compiler.compile(intent('GenericCard', { title: 'System Status' }), {
      agent: 'integration-test',
    });
    expect(result.status).toBe('pass');
    expect(result.errors).toHaveLength(0);
  ...
  it('all 20 results include provenance with correct agent', async () => {
    const result = await compiler.compile(intent('GenericCard', { title: 'Provenance check' }), {
