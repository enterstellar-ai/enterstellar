...
/**
 * @module @enterstellar/react/enterstellar-zone
...
import { EnterstellarError, createZoneId } from '@enterstellar/types';
...
import type { LifecycleManager, LifecycleState, StreamingAssembler } from '@enterstellar/lifecycle';

import {
  EnterstellarContext,
  EnterstellarAgentContext,
...
import type { ZoneProps } from './types.js';
import { DEFAULT_RETRY_POLICY, DEFAULT_AGENT_TIMEOUT_MS } from './types.js';
...
function isZoneIntentEvent(data: unknown): data is ZoneIntentEvent {
  ...
  return (
    typeof obj['zone'] === 'string' && typeof obj['intent'] === 'object' && obj['intent'] !== null
  );
}
 ...
 * @internal
 */
function getRetryDelay(attempt: number, backoff: 'exponential' | 'linear' | 'none'): number {
  switch (backoff) {
    case 'exponential': {
...
export function Zone(props: ZoneProps): React.JSX.Element {
  ...
  const compileIntent = useCallback(
    async (intent: ComponentIntent, attempt = 0): Promise<void> => {
      ...
      const currentCompilationId =
        attempt === 0 ? ++compilationIdRef.current : compilationIdRef.current;
      ...
      try {
        ...
        if (result.status === 'fail') {
          ...
          if (adapters.error?.sanitize !== undefined) {
            try {
              ...
              sanitizedError =
                sanitizeResult instanceof EnterstellarError
                  ? sanitizeResult
        ...
        if (adapters.error?.shouldRetry !== undefined) {
          try {
            const errForAdapter =
              err instanceof EnterstellarError
                ? err
                : new EnterstellarError(
                    'ENS-3005',
                    'react',
  ...
  const streamingProps: Readonly<Record<string, unknown>> =
    renderState.phase === 'streaming' ? renderState.partialProps : {};

  const streamingComponentName: string | null =
    renderState.phase === 'streaming' ? renderState.componentName : null;

  const currentError: Error | null = renderState.phase === 'error' ? renderState.error : null;

  // Map render phase to lifecycle state for LifecycleWrapper
  const lifecycleState: LifecycleState =
    renderState.phase === 'ready' ? 'ready' : renderState.phase;
