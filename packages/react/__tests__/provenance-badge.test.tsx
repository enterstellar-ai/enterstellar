/**
 * @module @enterstellar/react/__tests__/provenance-badge.test
...
import type { CompilationProvenance } from '@enterstellar/types';
...
describe('<ProvenanceBadge>', () => {
  it('renders nothing when visible={false}', () => {
    const { container } = render(<ProvenanceBadge provenance={makeProvenance()} visible={false} />);

    expect(container.innerHTML).toBe('');
  });

  it('renders badge when visible={true}', () => {
    const { container } = render(<ProvenanceBadge provenance={makeProvenance()} visible={true} />);
  ...
  it('displays the agent name', () => {
    const { container } = render(
      <ProvenanceBadge provenance={makeProvenance({ agent: 'claude-sonnet' })} visible={true} />,
    );
  ...
  it('shows forge mode in title when present', () => {
    const { container } = render(
      <ProvenanceBadge provenance={makeProvenance({ forgeMode: 'local' })} visible={true} />,
    );
