/**
 * @module @enterstellar/react/__tests__/defaults/generic-card.test
 ...
 * Mirrors the `CompilationError` shape from `@enterstellar/types/compiler.ts`.
 */
...
describe('GenericCard', () => {
  ...
  it('renders error messages with field paths', () => {
    ...
    expect(screen.getByText(/Expected enum value.*received "extreme"/)).toBeDefined();
  });

  it('renders fallback message when errors array is empty', () => {
    render(<GenericCard originalComponent="UnknownWidget" errors={[]} originalProps={{}} />);

    expect(screen.getByText(/Compilation failed — no error details available/)).toBeDefined();
  });
  ...
  it('hides props toggle when originalProps is empty', () => {
    render(
      <GenericCard originalComponent="EmptyComponent" errors={MOCK_ERRORS} originalProps={{}} />,
    );
  ...
  it('handles circular references in props gracefully', () => {
    ...
    const { container } = render(
      <GenericCard originalComponent="CircularComponent" errors={[]} originalProps={circular} />,
    );
