/**
 * @module @enterstellar/react/__tests__/enterstellar-zone.test
...
describe('<Zone>', () => {
  ...
  describe('context requirement (RE5)', () => {
    it('throws when rendered outside Provider', () => {
      ...
      }).toThrow('<Zone name="orphan"> must be rendered inside an <Provider>.');
    });
  });
  ...
  describe('wrapper div (RE8)', () => {
    ...
    it('passes style to wrapper div (with position:relative)', () => {
      ...
      const zoneDiv = container.querySelector(
        '[data-enterstellar-zone="styled-zone"]',
      ) as HTMLElement;
  ...
  describe('determinism rules', () => {
    ...
    it('determinism=1.0 (default) renders EnterstellarSkeleton initially (LC8)', () => {
      ...
      const hasSkeletonOrFallback = skeleton !== null || screen.queryByTestId('loading') !== null;
    ...
    it('default determinism is 1.0 (dynamic)', () => {
      const { container } = render(
        <TestWrapper>
          <Zone name="default-zone" fallback={<div data-testid="default-fallback">Loading</div>} />
        </TestWrapper>,
      );

      // Zone enters loading state on mount — either skeleton or fallback visible
      const skeleton = container.querySelector('[data-enterstellar-skeleton]');
      const hasSkeletonOrFallback =
        skeleton !== null || screen.queryByTestId('default-fallback') !== null;
      expect(hasSkeletonOrFallback).toBe(true);
    });
  });
  ...
  describe('fallback rendering', () => {
    it('renders EnterstellarSkeleton (LC8) when zone enters loading state', () => {
      const { container } = render(
        <TestWrapper>
          <Zone name="fallback-zone" fallback={<div data-testid="fb">Custom fallback</div>} />
        </TestWrapper>,
      );
