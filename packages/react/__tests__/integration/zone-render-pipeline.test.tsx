/**
 * @module @enterstellar/react/__tests__/integration/zone-render-pipeline.test
...
import type { ComponentIntent, CompilationResult } from '@enterstellar/types';
...
function createIntegrationMocks() {
  ...
  const compiler = {
    compile: vi.fn(
      async (intent: ComponentIntent): Promise<CompilationResult> => ({
  ...
  it('ignores intents targeted at other zones', async () => {
    ...
    render(
      <Provider
        ...
        connection={mocks.connection}
      >
        <Zone name="my-zone" fallback={<div data-testid="fallback">Waiting</div>} />
      </Provider>,
    );
  ...
  it('renders fallback when no renderer found for compiled component', async () => {
    ...
    render(
      <Provider
        ...
        connection={mocks.connection}
      >
        <Zone name="test-zone" fallback={<div data-testid="fallback">No renderer</div>} />
      </Provider>,
    );
  ...
  it('latest-intent-wins (P14) — new intent replaces previous', async () => {
    ...
    render(
      <Provider
        ...
        connection={mocks.connection}
      >
        <Zone name="p14-zone" fallback={<div data-testid="loading">Loading</div>} />
      </Provider>,
    );
  ...
  it('stores trace in EnterstellarStore on successful compilation (L4)', async () => {
    ...
    render(
      <Provider
        ...
        connection={mocks.connection}
      >
        <Zone name="traced-zone" fallback={<div>Loading</div>} />
      </Provider>,
    );
