/**
 * @module @enterstellar/react/__tests__/lifecycle-wrapper.test
 ...
 */
function makeContract(stateOverrides?: Partial<ComponentContract['states']>): ComponentContract {
  return {
    name: 'TestComponent',
...
describe('<LifecycleWrapper>', () => {
  ...
  describe('state="idle"', () => {
    it('renders null when no fallback is provided', () => {
      const { container } = render(<LifecycleWrapper {...makeProps({ state: 'idle' })} />);
  ...
  describe('state="loading"', () => {
    it('renders default EnterstellarSkeleton when no contract', () => {
      const { container } = render(<LifecycleWrapper {...makeProps({ state: 'loading' })} />);
  ...
  describe('state="error"', () => {
    ...
    it('renders custom error component when registered in contract (LC7)', () => {
      function CustomError(props: Record<string, unknown>): React.JSX.Element {
        return <div data-custom-error>Custom error: {(props['error'] as Error)?.message}</div>;
      }
  ...
  describe('state="empty"', () => {
    it('renders default EnterstellarEmptyState when no contract', () => {
      const { container } = render(<LifecycleWrapper {...makeProps({ state: 'empty' })} />);
