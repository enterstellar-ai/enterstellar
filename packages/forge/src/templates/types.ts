/**
 * @module @enterstellar/forge/templates/types
...
const TokenValueSchema = z
  .string()
  .refine((val): boolean => val.startsWith('token:'), {
 ...
 * - `@enterstellar/test` for template compliance assertions.
 *
...
export const ForgeTemplateSchema = z.object({
  ...
  categories: z.array(ComponentCategorySchema).min(1, 'At least one category is required.'),
