/**
 * @module @enterstellar/forge/__tests__/cloud-forge
...
function createValidContract(overrides: Partial<ComponentContract> = {}): ComponentContract {
  return {
    ...
    examples: [{ intent: 'show data', props: { title: 'hello' } }],
    _meta: {
      forged: false,
      version: '1.0.0',
...
describe('forgeCloud', () => {
  ...
  describe('successful generation', () => {
    ...
    it('invokes the callback with the intent and system prompt', async () => {
      ...
      const [receivedIntent, receivedPrompt] = (callback as ReturnType<typeof vi.fn>).mock
        .calls[0] as [ComponentIntent, string];
  ...
  describe('system prompt (F5)', () => {
    it('includes constraint information in the prompt', async () => {
      ...
      const prompt = (
        (callback as ReturnType<typeof vi.fn>).mock.calls[0] as [ComponentIntent, string]
      )[1];
  ...
  describe('callback failure handling (F9)', () => {
    ...
    it('does NOT throw on callback failure', async () => {
      ...
      await expect(forgeCloud(intent, constraints, callback)).resolves.toBeNull();
    });
  });
