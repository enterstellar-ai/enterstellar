/**
 * @module @enterstellar/adapter-supabase/__tests__/create-supabase-data-adapter
...
function createMockClient(options?: {
  ...
  const queryData =
    options?.queryData !== undefined ? options.queryData : [{ id: '1', name: 'Patient A' }];
  const queryError = options?.queryError !== undefined ? options.queryError : null;
  const mutateData =
    options?.mutateData !== undefined ? options.mutateData : { id: '1', name: 'Patient A' };
  ...
  const channelMock = {
    on: vi.fn().mockImplementation((_type: string, _filter: unknown, cb: () => Promise<void>) => {
