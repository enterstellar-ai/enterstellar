/**
 * @module @enterstellar/adapter-supabase/create-supabase-data-adapter
 ...
 * interface, then delegates to `createDataAdapter()` from `@enterstellar/adapters`
...
export function createSupabaseDataAdapter(config: SupabaseDataConfig): DataAdapter {
  ...
  return createDataAdapter({
    ...
    async mutate(
      ...
      // action === 'delete'
      const { error } = await client.from(resource).delete().eq('id', data['id']);
