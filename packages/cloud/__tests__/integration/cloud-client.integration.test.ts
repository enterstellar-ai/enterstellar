/**
 * @module @enterstellar/cloud/__tests__/integration/cloud-client.integration.test
...
describe('@enterstellar/cloud — Integration: Full Client Lifecycle', () => {
  ...
  describe('Scenario 1: Anonymous Flow', () => {
    it('create(pk_anon) → submitSignal → dispose', async () => {
      ...
      await expect(client.submitSignal({ intentHash: 'x' } as never)).rejects.toThrow(CloudError);
    });
  });
    ...
    it('forge → search → route → submitTrace → getUsage → getLedger → dispose', async () => {
      ...
      fetchMock.mockResolvedValueOnce(
        mockFetchResponse(
          200,
          {
            results: [{ componentName: 'VitalsCard', similarity: 0.92, contract: {} }],
          },
          ipuHeaders(11, 989, 1),
        ),
      );
      ...
      fetchMock.mockResolvedValueOnce(
        mockFetchResponse(
          200,
          {
            predictions: [{ componentName: 'VitalsCard', confidence: 0.92 }],
      ...
      fetchMock.mockResolvedValueOnce(
        mockFetchResponse(
          200,
          {
            items: [{ operation: 'forge', ipu_cost: 10, timestamp: '2026-03-29T12:00:00Z' }],
            cursor: null,
            hasMore: false,
          },
          ipuHeaders(12, 988, 0),
  ...
  describe('Scenario 3: Retry — 5xx → Retry → 2xx', () => {
    it('retries 5xx and succeeds, preserving same idempotency key', async () => {
      ...
      // First attempt: 500.
      fetchMock.mockResolvedValueOnce(mockFetchResponse(500, null, {}));
    ...
    it('ipu metadata flows from transport → proxy → CloudResult', async () => {
      ...
      fetchMock.mockResolvedValueOnce(
        mockFetchResponse(
          200,
          {
            results: [{ componentName: 'VitalsCard', similarity: 0.95, contract: {} }],
          },
          {
            'X-IPU-Used': '42',
            'X-IPU-Remaining': '958',
