/**
 * @module @enterstellar/cloud/__tests__/routing/cloud-router-proxy.test
  ...
  return {
    ...
    getEstimate: vi
      ...
      .mockReturnValue({
        ...
        remaining: 1000,
        limit: 1000,
        lastReconciliationCorrected: false,
      }),
  ...
  predictions: [
    {
      ...
      confidence: 0.92,
      registryUrl: 'https://registry.enterstellar.dev/vitals-card',
    },
...
describe('@enterstellar/cloud — CloudRouterProxy', () => {
  ...
  describe('route() — Single Intent (IR2)', () => {
    ...
    it('sends POST /v1/route with intentHash in body', async () => {
      ...
      const requestConfig = (transport.request as ReturnType<typeof vi.fn>).mock
  ...
  describe('routeBatch() — Batch Routing (IR5)', () => {
    ...
    const BATCH_PREDICTIONS: readonly RouterPrediction[] = [
      {
        predictions: [{ componentName: 'CardA', confidence: 0.9 }],
        metadata: { modelVersion: 'freq-v1', signalCount: 100 },
      },
      {
        predictions: [{ componentName: 'CardB', confidence: 0.8 }],
        metadata: { modelVersion: 'freq-v1', signalCount: 200 },
      },
      {
        predictions: [{ componentName: 'CardC', confidence: 0.7 }],
        metadata: { modelVersion: 'freq-v1', signalCount: 300 },
      },
    ];
    ...
    it('preserves input order (F19): data[i] ↔ intentHashes[i]', async () => {
      ...
      for (let i = 0; i < BATCH_HASHES.length; i++) {
        ...
        expect(prediction?.predictions[0]?.componentName).toBe(
          expected?.predictions[0]?.componentName,
        );
      }
    });
