/**
 * @module @enterstellar/cloud/__tests__/anonymous-mode.test
 ...
 * @see Design Choice SD4 — `@enterstellar/telemetry` uses SDK with `pk_anon_*`.
 */
...
describe('@enterstellar/cloud — Anonymous Mode (SD1)', () => {
  ...
  describe('All Other Methods — Throw ENS-5004', () => {
     ...
     */
    async function expectAUR5004(fn: () => Promise<unknown>, methodName: string): Promise<void> {
      try {
        await fn();
    ...
    it('forge() throws ENS-5004', async () => {
      await expectAUR5004(() => client.forge({ intent: 'show vitals' }), 'forge');
    });
    ...
    it('search() throws ENS-5004', async () => {
      await expectAUR5004(() => client.search('patient vitals'), 'search');
    });

    it('route() throws ENS-5004', async () => {
      await expectAUR5004(() => client.route('abcdef123456'), 'route');
    });

    it('routeBatch() throws ENS-5004', async () => {
      await expectAUR5004(() => client.routeBatch(['hash_a', 'hash_b']), 'routeBatch');
    });
    ...
    it('submitTrace() throws ENS-5004', async () => {
      ...
      await expectAUR5004(() => client.submitTrace(trace), 'submitTrace');
    });

    it('getTraces() throws ENS-5004', async () => {
      await expectAUR5004(() => client.getTraces(), 'getTraces');
    });

    it('analytics() throws ENS-5004', async () => {
      await expectAUR5004(() => client.analytics({ queryType: 'intent_patterns' }), 'analytics');
    });
    ...
    it('getUsage() throws ENS-5004', async () => {
      await expectAUR5004(() => client.getUsage(), 'getUsage');
    });

    it('getLedger() throws ENS-5004', async () => {
      await expectAUR5004(() => client.getLedger(), 'getLedger');
    });

    it('certify() throws ENS-5004', async () => {
      await expectAUR5004(() => client.certify('comp_01HYX'), 'certify');
    });

    it('deleteProjectData() throws ENS-5004', async () => {
      await expectAUR5004(() => client.deleteProjectData('proj_01'), 'deleteProjectData');
    });
  });
