/**
 * @module @enterstellar/cloud/__tests__/transport/cloud-http.test
 ...
 */
function mock429Response(upgradeUrl?: string, retryAfterMs?: number): Response {
  const errorBody = {
    error: {
      code: 'ENS-C4290',
...
describe('@enterstellar/cloud — CloudHttpTransport', () => {
  ...
  describe('IPU Header Parsing (§9.3)', () => {
    ...
    it('parses X-Request-Id from headers', async () => {
      fetchMock.mockResolvedValue(mockResponse(200, {}, { 'X-Request-Id': 'req_01HYX_abc' }));
    ...
    it('returns undefined for negative IPU header values', async () => {
      fetchMock.mockResolvedValue(mockResponse(200, {}, { 'X-IPU-Used': '-5' }));
  ...
  describe('Throw on 4xx (non-429) — No Retry', () => {
    ...
    it('throws CloudError on 401 — no retry', async () => {
      fetchMock.mockResolvedValue(mock4xxResponse(401, 'UNAUTHORIZED', 'Invalid API key'));
    ...
    it('includes parsed error code and requestId', async () => {
      fetchMock.mockResolvedValue(mock4xxResponse(403, 'FORBIDDEN', 'Access denied'));
