/**
 * @module @enterstellar/cloud/__tests__/analytics/cloud-analytics-proxy.test
...
describe('@enterstellar/cloud — CloudAnalyticsProxy', () => {
  ...
  describe('analytics() — Trace Analytics (TA3, TA5)', () => {
    ...
    it('sends POST /v1/traces/analytics (F17)', async () => {
      ...
      const config = (transport.request as ReturnType<typeof vi.fn>).mock.calls[0]?.[0] as Record<
        string,
        unknown
      >;
    ...
    it('throws CloudError on pre-flight quota exceeded', async () => {
      const overQuotaProxy = createCloudAnalyticsProxy(transport, createMockTracker(true), false);

      await expect(overQuotaProxy.analytics({ queryType: 'intent_patterns' })).rejects.toThrow(
        CloudError,
      );
