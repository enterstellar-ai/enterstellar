/**
 * @module @enterstellar/cloud/__tests__/create-cloud-client.test
...
describe('@enterstellar/cloud — createEnterstellarCloudClient()', () => {
  ...
  describe('Default Configuration', () => {
    it('uses default baseUrl https://api.enterstellar.dev when not specified (SD8)', async () => {
      fetchMock.mockResolvedValue(
        mockFetchResponse(
          200,
          { used: 0, limit: 1000, tier: 'starter' },
          {
            'X-IPU-Used': '0',
  ...
  describe('Disposal Lifecycle (ENS-5002)', () => {
    ...
    it('throws CloudError ENS-5002 when forge() called after dispose', async () => {
      ...
      await expect(client.forge({ intent: 'show vitals' })).rejects.toThrow(CloudError);
    ...
    it('throws ENS-5002 when businessAnalytics() called after dispose', async () => {
      client.dispose();
      await expect(client.businessAnalytics({ queryType: 'anomalies' })).rejects.toThrow(
        CloudError,
      );
    });
  });
