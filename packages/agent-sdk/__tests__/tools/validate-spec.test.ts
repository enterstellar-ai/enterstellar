/**
 * @module @enterstellar/agent-sdk/__tests__/tools/validate-spec
...
function createResult(
  ...
  return {
    ...
    errors:
      status === 'fail' ? [{ code: 'ENS-2001', path: 'props', message: 'Validation failed' }] : [],
 ...
 */
function createSpec(zones: Array<{ name: string; component: string }>): UISpec {
  return {
    zones: zones.map((z) => ({
      name: z.name,
...
describe('executeValidateSpec', () => {
  ...
  describe('multi-zone aggregation', () => {
    ...
    it('returns fail over corrected (fail takes precedence)', async () => {
      const compiler = createMockCompiler([createResult('corrected'), createResult('fail')]);
      const spec = createSpec([
        { name: 'main', component: 'A' },
        { name: 'sidebar', component: 'B' },
      ]);
  ...
  describe('L3 enforcement', () => {
    it('calls compiler.compile() once per zone', async () => {
      const compiler = createMockCompiler([createResult('pass'), createResult('pass')]);
      const spec = createSpec([
        { name: 'main', component: 'A' },
        { name: 'sidebar', component: 'B' },
      ]);
