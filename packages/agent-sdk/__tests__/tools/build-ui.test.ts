/**
 * @module @enterstellar/agent-sdk/__tests__/tools/build-ui
...
describe('executeBuildUI', () => {
  ...
  describe('full chain', () => {
    ...
    it('calls search with topK matching zone count', async () => {
      ...
      await executeBuildUI(semanticIndex, registry, compiler, 'patient data', zones);
  ...
  describe('auto-fill', () => {
    it('fills empty component fields from search results by position', async () => {
      ...
      const result = await executeBuildUI(semanticIndex, registry, compiler, 'patient data', zones);
