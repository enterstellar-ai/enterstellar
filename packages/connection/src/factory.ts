/**
 * @module @enterstellar/connection/factory
...
import { EnterstellarError, UserSignalSchema } from '@enterstellar/types';
...
import { backpressureDropWarning, configValidationError } from './errors.js';
...
function resolveConfig(input: ConnectionInput): {
  ...
  if (!parsed.success) {
    throw configValidationError(`Invalid connection config: ${parsed.error.message}`, parsed.error);
  }
 ...
 */
function classifyEvent(data: unknown): { event: AgentEventType; payload: unknown } | null {
 ...
 * ```
 */
export function createAgentConnection(input: ConnectionInput): EnterstellarAgentConnection {
  ...
  function wireTransport(t: Transport): void {
    t.onMessage((data: unknown) => {
      ...
      if (classified.event === 'intent') {
        ...
        if (result.dropped !== null) {
          ...
          console.warn(`[@enterstellar/connection] ${warning.message}`);
        }
    ...
    t.onError((error: unknown) => {
      ...
      console.error('[@enterstellar/connection] Transport error:', error);
    });
  }
  ...
  const connection: EnterstellarAgentConnection = {
    dispatch(signal: UserSignal, options?: { readonly immediate?: boolean }): Promise<void> {
