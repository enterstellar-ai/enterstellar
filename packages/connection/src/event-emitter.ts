/**
 * @module @enterstellar/connection/event-emitter
...
type TypedEventEmitter<TEvents extends Record<string, unknown>> = {
   ...
   */
  readonly emit: <K extends keyof TEvents & string>(event: K, data: TEvents[K]) => void;
   ...
   */
  readonly listenerCount: (event: keyof TEvents & string) => number;
};
    ...
    emit<K extends keyof TEvents & string>(event: K, data: TEvents[K]): void {
      ...
      for (const handler of handlers) {
        try {
          ...
          console.error(`[@enterstellar/connection] Event handler for '${event}' threw:`, error);
        }
      }
    },
