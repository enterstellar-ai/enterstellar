/**
 * @module @enterstellar/connection/__tests__/polling-transport.test
 ...
 */
function mockResponse(options: { ok?: boolean; status?: number; body?: unknown }): Response {
...
describe('createPollingTransport', () => {
  ...
  describe('connect()', () => {
    ...
    it('should reject when initial GET returns non-OK status', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockResponse({ ok: false, status: 503 })));
    ...
    it('should route poll HTTP errors to error handlers without stopping', async () => {
      const fetchMock = vi
        ...
        .mockResolvedValueOnce(
          mockResponse({ ok: true, body: [{ type: 'message', payload: 'recovered' }] }),
        );
      vi.stubGlobal('fetch', fetchMock);
  ...
  describe('send()', () => {
    it('should POST to the dispatch endpoint', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://test.example.com/poll/dispatch', {
        method: 'POST',
