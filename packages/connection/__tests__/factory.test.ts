/**
 * @module @enterstellar/connection/__tests__/factory.test
...
import type { UserSignal } from '@enterstellar/types';
...
describe('createAgentConnection', () => {
  describe('config validation', () => {
    it('should throw ENS-3001 for missing URL', () => {
      expect(() => createAgentConnection({ url: '' })).toThrow(EnterstellarError);
    });
  ...
  describe('dispatch()', () => {
    it('should serialize and send a valid UserSignal', async () => {
      ...
      const sent = JSON.parse(mockSocket.send.mock.calls[0]?.[0] as string) as Record<
        string,
        unknown
      >;
