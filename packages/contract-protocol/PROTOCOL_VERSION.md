...
The version of `@enterstellar/contract-protocol` is **permanently locked** to `@enterstellar/types`.

If `@enterstellar/types` is `1.2.0`, then `@enterstellar/contract-protocol` **must** be `1.2.0`.

This is a non-negotiable invariant. There is no independent versioning for this package.

## Rationale

`@enterstellar/contract-protocol` is a **serialized artifact** of `@enterstellar/types`. Every schema in `schemas/` is generated directly from a Zod schema defined in `@enterstellar/types` (except `design-tokens-dtcg.json`, which is hand-crafted but still versioned with the package). Independent versioning would create:
...
1. Bump `@enterstellar/types` version (e.g., `0.0.0` → `1.0.0`).
2. Set `@enterstellar/contract-protocol` version to the **exact same value** (`1.0.0`).
3. Run `pnpm --filter @enterstellar/contract-protocol run generate` to regenerate schemas.
