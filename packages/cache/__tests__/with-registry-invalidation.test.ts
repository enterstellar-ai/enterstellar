/**
 * @module @enterstellar/cache/__tests__/with-registry-invalidation
...
import type { CompilationResult, ComponentIntent } from '@enterstellar/types';
...
describe('withRegistryInvalidation()', () => {
  it('returns the same cache instance (for chaining)', () => {
    ...
    const registry = createMockRegistry();
    const { cache: returnedCache } = withRegistryInvalidation(originalCache, registry);
...
describe('withRegistryInvalidation — "update" event', () => {
  it('invalidates all cache entries for the updated component', () => {
    ...
    cache.set(buildCacheKey('h1', 'PatientVitals'), makeCachedRender('PatientVitals'));
    cache.set(buildCacheKey('h2', 'PatientVitals'), makeCachedRender('PatientVitals'));
    cache.set(buildCacheKey('h3', 'MedicationList'), makeCachedRender('MedicationList'));
    ...
    expect(cache.get(buildCacheKey('h3', 'MedicationList'))).toBeDefined();
  });

  it('is a no-op when updated component has no cache entries', () => {
    const { cache, registry } = setupTest();

    cache.set(buildCacheKey('h1', 'CompA'), makeCachedRender('CompA'));
...
describe('withRegistryInvalidation — "unregister" event', () => {
  it('invalidates all cache entries for the unregistered component', () => {
    ...
    cache.set(buildCacheKey('h1', 'RemovedComp'), makeCachedRender('RemovedComp'));
    cache.set(buildCacheKey('h2', 'KeptComp'), makeCachedRender('KeptComp'));

    registry.fire('unregister', makeContract('RemovedComp'));

    expect(cache.size).toBe(1);
    expect(cache.get(buildCacheKey('h2', 'KeptComp'))).toBeDefined();
  });
});
...
describe('withRegistryInvalidation — "register" event', () => {
  it('does NOT invalidate cache on new component registration', () => {
    ...
    cache.set(buildCacheKey('h1', 'ExistingComp'), makeCachedRender('ExistingComp'));

    // Register a new component — should NOT affect cache
    registry.fire('register', makeContract('NewComponent'));

    expect(cache.size).toBe(1); // Unchanged
    expect(cache.get(buildCacheKey('h1', 'ExistingComp'))).toBeDefined();
  });
});
