/**
 * @module @enterstellar/cache/__tests__/create-render-cache
 ...
 */
function makeCachedRender(componentName: string, ttlSeconds: number = 3600): CachedRender {
  const now = Date.now();
  return {
...
describe('RenderCache — invalidateByComponent()', () => {
  ...
  it('calls onEvict with reason "component-update" for each evicted entry', () => {
    ...
    expect(onEvict).toHaveBeenCalledWith(buildCacheKey('h1', 'Target'), 'component-update');
    expect(onEvict).toHaveBeenCalledWith(buildCacheKey('h2', 'Target'), 'component-update');
  });
...
describe('RenderCache — warmup()', () => {
  ...
  it('skips failed compilations (status === "fail")', async () => {
    ...
    await cache.warmup([{ zone: 'test', intent: makeIntent('FailedComp') }], compile);
  ...
  it('caches corrected compilations', async () => {
    ...
    await cache.warmup([{ zone: 'test', intent: makeIntent('CorrectedComp') }], compile);
