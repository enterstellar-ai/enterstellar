/**
 * @module @enterstellar/semantic-index/create-semantic-index
...
export function createSemanticIndex(config: SemanticIndexConfig): SemanticIndex {
  ...
  async function embedComponent(contract: ComponentContract): Promise<void> {
    ...
    try {
      ...
      throw embeddingProviderError(`Failed to embed component '${contract.name}'`, error);
    }
  }
  ...
  return {
    async build(): Promise<void> {
      ...
      if (contracts.length > 0) {
        ...
        try {
          ...
          throw embeddingProviderError('Batch embedding failed during build()', error);
        }
      }
    ...
    async search(
      ...
      try {
        ...
        throw embeddingProviderError(`Failed to embed intent: '${intent}'`, error);
      }
