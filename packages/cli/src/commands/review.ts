/**
 * @module @enterstellar/cli/commands/review
...
export async function reviewCommand(
  ...
  const scanPaths = pathArgs.length > 0 ? pathArgs.map((p) => resolve(p)) : [resolve('.')];
  ...
  const allFileAnnotations = await Promise.all(
    contractFiles.map(async (filePath) => {
      try {
        ...
        console.error(pc.dim(`  ⚠ Could not read ${filePath}: ${message}`));
        return null;
      }
    }),
  );
  ...
  const annotatedFiles = allFileAnnotations.filter(
    (f): f is NonNullable<typeof f> => f !== null && f.annotations.length > 0,
  );
