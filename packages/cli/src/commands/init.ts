/**
 * @module @enterstellar/cli/commands/init
...
export async function initCommand(directoryArg?: string): Promise<void> {
  ...
  // runInstall() (so @enterstellar/migration is available in node_modules).
 ...
 */
async function collectOptions(directoryArg?: string): Promise<InitOptions> {
  // --- Project Name ---
  ...
  const directory = resolve(
    directoryArg !== undefined && directoryArg.length > 0 ? directoryArg : projectName,
  );
 ...
 * 1. Dynamic-imports `scanComponentsLightweight` from `@enterstellar/migration`
 ...
 */
async function handleExistingProjectDetection(options: InitOptions): Promise<InitOptions> {
  if (!detectExistingReactProject(options.directory)) {
    return options;
  }

  p.log.info(`${pc.yellow('Existing React project detected')} in ${pc.dim(options.directory)}`);

  // Dynamic-import @enterstellar/migration for the lightweight scan.
  ...
  const { scanComponentsLightweight } = await import('@enterstellar/migration');
  ...
  const reactVersionStr =
    result.reactVersion !== undefined ? `React ${result.reactVersion}` : 'React (version unknown)';
  ...
  if (migrationChoice === 'no') {
    p.log.info(pc.dim("Run 'enterstellar migrate src/' when you're ready to generate contracts."));
  } else {
...
async function writeScaffoldFiles(options: InitOptions): Promise<void> {
  ...
  await safeWriteFile(resolve(directory, 'tsconfig.json'), generateTsconfig(template));

  await safeWriteFile(resolve(directory, 'README.md'), generateReadme(projectName, packageManager));

  // --- Enterstellar registry and tokens ---
  await safeWriteFile(resolve(directory, 'src', 'enterstellar', 'registry.ts'), generateRegistry());

  await safeWriteFile(resolve(directory, 'src', 'enterstellar', 'tokens.ts'), generateTokens());

  await safeWriteFile(resolve(directory, 'src', 'globals.css'), generateGlobalsCss());

  // --- Example components ---
  const components = generateComponents();
  for (const { filename, content } of components) {
    await safeWriteFile(resolve(directory, 'src', 'enterstellar', 'components', filename), content);
  }

  // --- App page ---
  const pagePath = getPagePath(template);
  await safeWriteFile(resolve(directory, pagePath), generatePage(template));

  // --- Tests ---
  await safeWriteFile(resolve(directory, 'src', 'tests', 'enterstellar.test.ts'), generateTest());
...
function printSuccess(options: InitOptions): void {
  ...
  p.note(
    [
      ...
      `  npx @enterstellar/cli add component MyComponent  ${pc.dim('— Scaffold a new component')}`,
    ].join('\n'),
    'Success!',
  );
