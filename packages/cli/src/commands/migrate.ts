/**
 * @module @enterstellar/cli/commands/migrate
 * @description Implements the `enterstellar migrate <path> [flags]` command.
 *
 * This is the **CLI orchestrator** — the glue layer that wires:
 * - `@enterstellar/migration` (domain logic: extract, enrich, assemble)
 * - `@enterstellar/cli/migrate/*` (file discovery, outcome determination, formatters)
 ...
 *   ├─ 4. Dynamic import (@enterstellar/migration + ts-morph)
 ...
 *
 * - **Dynamic imports (Audit W4):** `@enterstellar/migration` and `ts-morph` are
...
export async function migrateCommand(
  ...
  // @enterstellar/migration (and its ts-morph dep) are loaded only when
  ...
  const {
    ...
    enrichManifest,
    resolveProvider,
  } = await import('@enterstellar/migration');
 ...
 *
 * Function types are defined structurally to match `@enterstellar/migration`'s
...
type ProcessFileContext = {
   ...
   */
  readonly extractManifest: (source: string, filename?: string, project?: unknown) => ExtractResult;
  ...
  readonly assembleTest: (manifest: StructuralManifest, contractImportPath: string) => string;
...
async function processFile(
  ...
  const componentBasename =
    sourceBasename.length < sourceBasenameTs.length ? sourceBasename : sourceBasenameTs;
  ...
  try {
    ...
    const reason = message.startsWith('SKIP: ') ? message.slice(6) : message;
  ...
  if (flags.enrich && !flags.dryRun && ctx.enrichmentProvider !== undefined) {
    ...
    if (enrichResult.enrichedFields.length > 0) {
      assemblyOptions = {
        ...
        ...(flags.provider !== undefined ? { enrichmentProvider: flags.provider } : {}),
      };
    }
  }

  // Use the enriched manifest if available, otherwise the original.
  const finalManifest = enrichResult !== undefined ? enrichResult.manifest : manifest;
 ...
 */
function resolveOutputDir(filePath: string, outDir: string, primaryInputPath: string): string {
  const sourceDir = dirname(filePath);
