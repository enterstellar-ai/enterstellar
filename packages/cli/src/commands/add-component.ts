/**
 * @module @enterstellar/cli/commands/add-component
 ...
 * Enterstellar project root (nearest `package.json` with `@enterstellar/registry` dependency).
 *
...
import { createInvalidComponentNameError, createProjectNotFoundError } from '../utils/errors.js';
 ...
 *    `@enterstellar/registry` in dependencies or devDependencies (ENS-9004 if not found).
 ...
 */
export async function addComponentCommand(componentName: string): Promise<void> {
 ...
 * that contains `@enterstellar/registry` as a dependency or devDependency.
 *
 ...
 *
 * An Enterstellar project is identified by having `@enterstellar/registry` listed
...
function isEnterstellarProject(packageJsonPath: string): boolean {
  try {
    ...
    if (typeof deps === 'object' && deps !== null && '@enterstellar/registry' in deps) {
      return true;
    }

    if (typeof devDeps === 'object' && devDeps !== null && '@enterstellar/registry' in devDeps) {
      return true;
    }
