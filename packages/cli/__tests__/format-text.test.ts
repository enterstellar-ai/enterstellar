/**
 * @module @enterstellar/cli/__tests__/format-text
...
import { formatBatchSummaryText, formatResultText } from '../src/migrate/format-text.js';
...
describe('formatBatchSummaryText', () => {
  ...
  it('renders warn count line when warnCount > 0', () => {
    const output = stripAnsi(formatBatchSummaryText(makeSummary({ warnCount: 3 }), 'src/'));
    expect(output).toContain('3 contracts generated');
    expect(output).toContain('(warnings)');
  });

  it('renders review count line when reviewCount > 0', () => {
    const output = stripAnsi(formatBatchSummaryText(makeSummary({ reviewCount: 7 }), 'src/'));
