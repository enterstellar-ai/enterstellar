/**
 * @module @enterstellar/cli/__tests__/e2e-scaffold
...
/**
 * Core `@enterstellar/*` dependencies that MUST be present in every scaffolded
 * `package.json`, regardless of template variant.
 *
 * Engine packages (@enterstellar/compiler, @enterstellar/state, @enterstellar/telemetry, etc.)
 * are transitive dependencies of @enterstellar/react per RE19 and CLI4.
 */
...
/**
 * Additional `@enterstellar/*` dev dependencies present in `full`, `nextjs`, and
...
async function scaffoldProject(template: ProjectTemplate): Promise<string> {
  ...
  await safeWriteFile(join(projectDir, 'src', 'globals.css'), generateGlobalsCss());
  ...
  for (const template of ALL_TEMPLATES) {
    describe(`template: "${template}"`, () => {
      ...
      describe('package.json structural validation', () => {
        ...
        it('has all core @enterstellar/* dependencies', () => {
          const pkg = readJsonFile(projectDir, 'package.json');
        ...
        it('has extended @enterstellar/* dev deps for non-minimal templates', () => {
      ...
      describe('page component validation', () => {
        ...
        it('imports Provider and Zone from @enterstellar/react', () => {
          const pagePath = getPagePath(template);
          const page = readTextFile(projectDir, pagePath);

          expect(page).toContain('Provider');
          expect(page).toContain('Zone');
          expect(page).toContain('@enterstellar/react');
        });
        ...
        it('uses Provider with auto-created compiler (RE1, RE19)', () => {
          ...
          // Provider auto-creates compiler — no @enterstellar/compiler import
          expect(page).not.toContain('@enterstellar/compiler');
