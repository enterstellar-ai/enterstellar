/**
 * @module @enterstellar/cli/__tests__/resolve-source-files
...
function createTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `enterstellar-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe('resolveSourceFiles', () => {
  ...
  it('deduplicates files from overlapping paths', async () => {
    ...
    const { files } = await resolveSourceFiles([filePath, join(root, 'src')], []);
    expect(files).toHaveLength(1);
  });
  ...
  it('applies --exclude patterns (Layer 3)', async () => {
    ...
    const { files } = await resolveSourceFiles([join(root, 'src')], ['**/*.stories.tsx']);
  ...
  it('returns empty files array for non-existent path', async () => {
    const { files } = await resolveSourceFiles(['/tmp/__nonexistent_enterstellar_path__'], []);
  ...
  it('handles glob patterns directly', async () => {
    ...
    const { files } = await resolveSourceFiles([`${root}/src/**/*.tsx`], []);
    expect(files).toHaveLength(2);
  });
});
