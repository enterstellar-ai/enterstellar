/**
 * @module @enterstellar/cli/__tests__/add-component
 ...
 * - Enterstellar project detection (package.json with @enterstellar/registry)
...
beforeEach(() => {
  testDir = join(
    tmpdir(),
    `enterstellar-cli-test-add-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
describe('Enterstellar project detection', () => {
  it('identifies a project with @enterstellar/registry in dependencies', () => {
    const pkg = {
      name: 'my-enterstellar-app',
      dependencies: { '@enterstellar/registry': 'latest' },
    };
    ...
    expect('@enterstellar/registry' in (deps as Record<string, unknown>)).toBe(true);
  });

  it('identifies a project with @enterstellar/registry in devDependencies', () => {
    const pkg = {
      name: 'my-enterstellar-app',
      devDependencies: { '@enterstellar/registry': 'latest' },
    };
    writeFileSync(join(testDir, 'package.json'), JSON.stringify(pkg));
    ...
    expect('@enterstellar/registry' in (devDeps as Record<string, unknown>)).toBe(true);
  });

  it('rejects a project without @enterstellar/registry', () => {
    const pkg = {
      name: 'plain-app',
    ...
    expect(typeof deps).toBe('object');
    expect(deps).not.toBeNull();
    expect('@enterstellar/registry' in (deps as Record<string, unknown>)).toBe(false);
  });
