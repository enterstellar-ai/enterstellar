/**
 * @module @enterstellar/cli/__tests__/init-command
...
beforeEach(() => {
  testDir = join(
    tmpdir(),
    `enterstellar-cli-test-init-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
...
describe('Full scaffold simulation', () => {
  ...
  for (const template of TEMPLATES) {
    it(`scaffolds all required files for "${template}" template`, async () => {
      ...
      expect(
        existsSync(join(projectDir, 'src', 'enterstellar', 'components', 'ExampleCard.tsx')),
      ).toBe(true);
      expect(
        existsSync(join(projectDir, 'src', 'enterstellar', 'components', 'ExampleList.tsx')),
      ).toBe(true);
      expect(
        existsSync(join(projectDir, 'src', 'enterstellar', 'components', 'ExampleChart.tsx')),
      ).toBe(true);
      expect(
        existsSync(join(projectDir, 'src', 'enterstellar', 'components', 'ExampleForm.tsx')),
      ).toBe(true);
      expect(
        existsSync(join(projectDir, 'src', 'enterstellar', 'components', 'ExampleDetail.tsx')),
      ).toBe(true);
    });
  }
});
