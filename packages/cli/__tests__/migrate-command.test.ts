/**
 * @module @enterstellar/cli/__tests__/migrate-command
...
function createTempDir(prefix: string): string {
  const dir = join(
    tmpdir(),
    `enterstellar-migrate-test-${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe('migrateCommand', () => {
  ...
  it('exits 1 when --update flag is used (Audit W3 stub)', async () => {
    ...
    await migrateCommand([join(root, 'src')], [join(root, 'src'), '--update']);
    expect(process.exitCode).toBe(1);
    const errorOutput = errorSpy.mock.calls.map((c) => String(c[0])).join(' ');
  ...
  it('handles empty file list gracefully (no error)', async () => {
    ...
    await migrateCommand([join(root, 'empty')], [join(root, 'empty')]);
  ...
  it('migrates a single valid component and writes contract + test files', async () => {
    ...
    await migrateCommand([join(root, 'src', 'Button.tsx')], [join(root, 'src', 'Button.tsx')]);
  ...
  it('mirrors source structure under --out directory', async () => {
    ...
    await migrateCommand([srcDir], [srcDir, '--out', outDir]);
