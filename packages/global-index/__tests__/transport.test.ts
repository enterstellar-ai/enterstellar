/**
 * @module @enterstellar/global-index/transport.test
 ...
 */
function mockNonJsonResponse(status: number = 200, statusText: string = 'OK'): Response {
  return {
    ok: status >= 200 && status < 300,
    status,
    statusText,
...
describe('execute — HTTP errors', () => {
  ...
  it('falls back to HTTP status text when body is not JSON', async () => {
    fetchMock.mockResolvedValueOnce(mockNonJsonResponse(502, 'Bad Gateway'));
