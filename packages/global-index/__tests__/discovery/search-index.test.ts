/**
 * @module @enterstellar/global-index/discovery/search-index.test
...
import { getContract, getFeatured, searchContracts } from '../../src/discovery/search-index.js';
...
describe('searchContracts', () => {
  it('returns search results on success', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ results: [MOCK_SEARCH_RESULT] }));
  ...
  it('returns multiple results sorted by relevance', async () => {
    ...
    fetchMock.mockResolvedValueOnce(mockResponse({ results: [MOCK_SEARCH_RESULT, second] }));
  ...
  it('throws ENS-5035 on malformed response', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ not_results: [] }));
...
describe('getContract', () => {
  it('returns a GlobalSearchResult on success', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ result: MOCK_SEARCH_RESULT }));
  ...
  it('returns null on 404 Not Found', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse(null, { status: 404, statusText: 'Not Found' }));
