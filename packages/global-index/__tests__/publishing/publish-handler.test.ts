/**
 * @module @enterstellar/global-index/publishing/publish-handler.test
...
describe('publishContract', () => {
  it('returns a GlobalSearchResult on success', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ result: MOCK_PUBLISH_RESULT }));
...
describe('getPublisherStats', () => {
  it('returns PublishEarnings on success', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ stats: MOCK_EARNINGS }));
  ...
  it('throws ENS-5035 on malformed server response', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ not_stats: {} }));
