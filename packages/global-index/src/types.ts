/**
 * @module @enterstellar/global-index/types
 ...
 *
 * This enables `@enterstellar/global-index` to accept any object that exposes
 * the cloud client's HTTP transport, without tightly coupling to the
 * `@enterstellar/cloud` implementation.
 */
...
export interface GlobalIndex {
   ...
   */
  search(query: string, options?: GlobalSearchOptions): Promise<readonly GlobalSearchResult[]>;
   ...
   */
  getContract(name: string, registryUrl: string): Promise<GlobalSearchResult | null>;
   ...
   * @see Design Choice GI1 — self-registration via `POST /v1/registries`.
   */
  registerRegistry(registration: RegistryRegistration): Promise<FederatedRegistry>;
   ...
   */
  publishContract(contract: ComponentContract): Promise<GlobalSearchResult>;
