/**
 * @module @enterstellar/test/__tests__/resolve
...
function createTestComponent(name: string = 'TestCard') {
  return defineComponent({
    ...
    examples: [{ intent: `show ${name.toLowerCase()}`, props: { title: 'Hello' } }],
  });
}
...
describe('harness.resolve()', () => {
  ...
  describe('unmocked intent', () => {
    it('throws EnterstellarError with code ENS-5010 for unmocked intent', async () => {
      ...
      await expect(harness.resolve('unmocked intent string')).rejects.toThrow(EnterstellarError);
