/**
 * @module @enterstellar/test/__tests__/intent-tests
 * @description P1 Gate — 10 intent-based tests using `@enterstellar/test`.
 *
 * These tests demonstrate the `@enterstellar/test` harness as a **consumer** would
 ...
 *
 * @see P1 Gate Checklist — "10 intent-based tests pass using `@enterstellar/test`"
...
const MedicationList = defineComponent({
  ...
  examples: [{ intent: 'display medication list', props: { patientId: 'P-002' } }],
});
...
const LabResults = defineComponent({
  ...
  examples: [{ intent: 'show lab results', props: { labId: 'LAB-100', category: 'blood' } }],
});
...
const AppointmentCard = defineComponent({
  ...
  examples: [{ intent: 'display appointment card', props: { appointmentId: 'APT-200' } }],
});
