/**
 * @module @enterstellar/test/harness-resolve
 ...
 * 2. Compile via real `@enterstellar/compiler` (Zod, tokens, a11y all run)
...
import type { AgentTrace, CompilationResult, ComponentIntent, TraceId } from '@enterstellar/types';
import { EnterstellarError, createTraceId } from '@enterstellar/types';
...
export async function resolve(
  ...
  const compilationResult: CompilationResult = await compiler.compile(mockResponse, {
 ...
 *
 * The trace follows the exact `AgentTrace` shape from `@enterstellar/types/trace`.
...
function buildTrace(
  ...
  const hasA11yErrors = result.errors.some((e) => e.code === 'ENS-2003');
  ...
  if (options?.context !== undefined) {
    ...
    // @enterstellar/types. We use the correlationId field for test traceability.
    return {
      ...trace,
