/**
 * @module @enterstellar/registry/__tests__/create-registry
...
describe('createRegistry()', () => {
  ...
  it('throws on duplicate names in initial components', () => {
    ...
    expect(() => createRegistry({ components: [contract, contract] })).toThrow(EnterstellarError);
    expect(() => createRegistry({ components: [contract, contract] })).toThrow(/ENS-1001/);
  });
});
...
describe('EnterstellarRegistry.list()', () => {
  it('returns sorted names of all registered components', () => {
    const registry = createRegistry({
      components: [makeContract('Zebra'), makeContract('Alpha'), makeContract('Mango')],
    });
...
describe('EnterstellarRegistry.getManifest()', () => {
  it('returns CompactManifestEntry[] for all registered components', () => {
    const registry = createRegistry({
      components: [makeContract('Alpha'), makeContract('Beta')],
    });
...
describe('EnterstellarRegistry.getDesignTokens()', () => {
  ...
  it('merges component-level tokens with config-level tokens', () => {
    const registry = createRegistry({
      components: [makeContract('WithTokens', { tokens: { compToken: 'token:comp-value' } })],
