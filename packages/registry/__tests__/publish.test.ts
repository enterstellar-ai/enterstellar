/**
 * @module @enterstellar/registry/__tests__/publish
 ...
 */
function createMockFetch(status: number, body: unknown, ok?: boolean): ReturnType<typeof vi.fn> {
  return vi.fn(() =>
    Promise.resolve({
...
describe('registry.publish()', () => {
  ...
  it('throws EnterstellarError ENS-5001 on HTTP 403 (forbidden)', async () => {
    ...
    await expect(registry.publish(contract, TEST_TARGET)).rejects.toThrow(EnterstellarError);

    await expect(registry.publish(contract, TEST_TARGET)).rejects.toThrow(/ForbiddenCard/);
  });
