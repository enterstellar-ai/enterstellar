/**
 * @module @enterstellar/registry/__tests__/define-component
...
describe('defineComponent()', () => {
  it('returns a frozen ComponentContract with auto-generated id and _meta', () => {
    ...
    expect(contract.description).toBe(
      'Displays real-time patient vital signs with risk stratification.',
    );
...
describe('defineComponent() — validation rules', () => {
  ...
  describe('R1 — PascalCase name', () => {
    it('rejects camelCase names', () => {
      expect(() => defineComponent(validInput({ name: 'patientVitals' }))).toThrow(
        EnterstellarError,
      );
    ...
    it('rejects snake_case names', () => {
      expect(() => defineComponent(validInput({ name: 'patient_vitals' }))).toThrow(
        EnterstellarError,
      );
    });

    it('rejects kebab-case names', () => {
      expect(() => defineComponent(validInput({ name: 'patient-vitals' }))).toThrow(
        EnterstellarError,
      );
    });
  ...
  describe('R2 — Description ≤ 120 characters', () => {
    it('rejects descriptions over 120 characters', () => {
      ...
      expect(() => defineComponent(validInput({ description: longDesc }))).toThrow(
        EnterstellarError,
      );
  ...
  describe('R7 — Props must be a Zod schema', () => {
    it('rejects null props', () => {
      expect(() => defineComponent(validInput({ props: null as unknown as z.ZodType }))).toThrow(
        EnterstellarError,
      );
      expect(() => defineComponent(validInput({ props: null as unknown as z.ZodType }))).toThrow(
        /ENS-1008/,
  ...
  describe('R6 — Token values must start with token:', () => {
    it('rejects raw CSS values', () => {
      expect(() => defineComponent(validInput({ tokens: { color: '#ff0000' } }))).toThrow(
        EnterstellarError,
      );
      expect(() => defineComponent(validInput({ tokens: { color: '#ff0000' } }))).toThrow(
        /ENS-1007/,
    ...
    it('rejects tokens without prefix', () => {
      expect(() => defineComponent(validInput({ tokens: { bg: 'danger' } }))).toThrow(
        EnterstellarError,
      );
    });
