/**
 * @module @enterstellar/registry/create-registry
...
export function createRegistry(config: RegistryConfig): EnterstellarRegistry {
  ...
  const registry: EnterstellarRegistry = {
    ...
    async publish(contract: ComponentContract, target: PublishTarget): Promise<PublishResult> {
      ...
      if (!validationResult.valid) {
        ...
        const message =
          firstViolation !== undefined ? firstViolation.message : 'Unknown validation error.';
        throw new EnterstellarError(
          'ENS-1002',
          'registry',
