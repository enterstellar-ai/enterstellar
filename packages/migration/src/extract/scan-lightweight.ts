/**
 * @module @enterstellar/migration/extract/scan-lightweight
 ...
 *
 * **Why in `@enterstellar/migration`?** `ts-morph` (~5MB) is a dependency of
 * `@enterstellar/migration`, NOT `@enterstellar/cli`. Placing this function here allows
...
function classifyVariableDeclaration(decl: VariableDeclaration): FileTier {
  ...
  if (
    initText.startsWith('forwardRef') ||
    initText.startsWith('React.forwardRef') ||
    initText.startsWith('memo') ||
    initText.startsWith('React.memo')
  ) {
    return 'review';
  }
 ...
 */
export function scanComponentsLightweight(srcDir: string): ComponentScanResult {
