/**
 * @module @enterstellar/migration/enrichment/cloud-provider
 ...
 *   `@enterstellar/cloud` metering pattern from `create-cloud-client.ts`).
 *
 * **`@enterstellar/cloud` patterns adopted:**
 ...
 * @see `@enterstellar/cloud/create-cloud-client.ts` — Cloud SDK patterns (auth, IPU, metering)
 */
 ...
 *
 * Follows `@enterstellar/cloud` patterns: bearer auth, `X-IPU-Remaining`
 * header tracking, and consistent error mapping.
 *
 * @see Correction 3 — two providers at launch
 * @see `@enterstellar/cloud/create-cloud-client.ts` — pattern reference
 */
export class CloudEnrichmentProvider implements EnrichmentProvider {
   ...
   *
   * Follows the `@enterstellar/cloud` metering pattern — IPU data is surfaced
