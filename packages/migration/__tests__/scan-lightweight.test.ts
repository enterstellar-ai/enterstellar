/**
 * @module @enterstellar/migration/__tests__/scan-lightweight
 ...
 * @see Audit E3 — scanner lives in @enterstellar/migration
 */
 ...
 */
function createTestProject(files: Record<string, string>, packageJsonContent?: string): void {
  // Write package.json
  if (packageJsonContent !== undefined) {
    writeFileSync(join(tempDir, 'package.json'), packageJsonContent);
  }
...
describe('scanComponentsLightweight', () => {
  ...
  it('classifies function components with explicit props as auto-migratable', async () => {
    ...
    createTestProject({ 'src/Button.tsx': AUTO_MIGRATABLE_COMPONENT }, REACT_PACKAGE_JSON);
  ...
  it('classifies arrow components with typed props as auto-migratable', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/Card.tsx': AUTO_MIGRATABLE_ARROW }, REACT_PACKAGE_JSON);
  ...
  it('classifies React.FC components as auto-migratable', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/Badge.tsx': AUTO_MIGRATABLE_FC }, REACT_PACKAGE_JSON);
  ...
  it('classifies generic components as manual review', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/List.tsx': MANUAL_REVIEW_GENERIC }, REACT_PACKAGE_JSON);
  ...
  it('classifies forwardRef components as manual review', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/Input.tsx': MANUAL_REVIEW_FORWARD_REF }, REACT_PACKAGE_JSON);
  ...
  it('classifies re-exports as manual review', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/index.tsx': MANUAL_REVIEW_REEXPORT }, REACT_PACKAGE_JSON);
  ...
  it('classifies utility files as skipped', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/utils.tsx': SKIPPED_UTILITY }, REACT_PACKAGE_JSON);
  ...
  it('classifies type-only files as skipped', async () => {
    const srcDir = join(tempDir, 'src');
    createTestProject({ 'src/types.tsx': SKIPPED_TYPES }, REACT_PACKAGE_JSON);
  ...
  it('returns undefined react version when react is not in deps', async () => {
    ...
    createTestProject({ 'src/Button.tsx': AUTO_MIGRATABLE_COMPONENT }, noReactPkg);
