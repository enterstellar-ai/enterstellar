/**
 * @module @enterstellar/migration/__tests__/build-prompt
 ...
 */
function createTestManifest(overrides?: Partial<StructuralManifest>): StructuralManifest {
  return {
    name: 'TestComponent',
...
describe('buildEnrichmentPrompt — structural context', () => {
  ...
  it('includes generic parameters when present', () => {
    const manifest = createTestManifest({
      generics: [{ name: 'T', constraint: 'Record<string, unknown>' }, { name: 'U' }],
    });
    const prompt = buildEnrichmentPrompt(manifest, SAMPLE_SOURCE, ['description']);
