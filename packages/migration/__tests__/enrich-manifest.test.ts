/**
 * @module @enterstellar/migration/__tests__/enrich-manifest
 ...
 */
function createManifest(astFields: readonly EnrichableFieldKey[] = []): StructuralManifest {
  const makeField = <T>(key: EnrichableFieldKey, value: T): EnrichableField<T> => ({
    value,
...
describe('enrichManifest — gating logic', () => {
  ...
  it('enriches only heuristic fields in mixed scenario', async () => {
    ...
    const overlay: SemanticOverlay = {
      fields: [{ key: 'description', value: 'Enhanced description' }],
    };
    const provider = createMockProvider(overlay);
...
describe('mergeOverlay', () => {
  it('promotes source to enrichment for patched fields', () => {
    const manifest = createManifest();
    const overlay: SemanticOverlay = {
      fields: [{ key: 'description', value: 'Enriched description' }],
    };
