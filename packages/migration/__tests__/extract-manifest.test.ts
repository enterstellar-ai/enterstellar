/**
 * @module @enterstellar/migration/__tests__/extract-manifest
...
describe('extractManifest — diagnostics', () => {
  ...
  it('emits info diagnostic for existing Zod schemas', () => {
    ...
    const schemaDiag = result.diagnostics.find((d) => d.message.includes('Existing Zod schemas'));
...
describe('extractManifest — project reuse', () => {
  it('accepts an external Project for batch reuse', () => {
    ...
    const r1 = extractManifest(`export function A() { return <div />; }`, 'A.tsx', sharedProject);
    const r2 = extractManifest(`export function B() { return <span />; }`, 'B.tsx', sharedProject);
  ...
  it('creates internal project when none provided (server mode)', () => {
    ...
    const result = extractManifest(`export function Solo() { return <div />; }`, 'Solo.tsx');
