/**
 * @module @enterstellar/adapter-firebase/create-firebase-data-adapter
 ...
 * then delegates to `createDataAdapter()` from `@enterstellar/adapters`
...
export function createFirebaseDataAdapter(config: FirebaseDataConfig): DataAdapter {
  ...
  return createDataAdapter({
    ...
    async query(
      ...
      const queryRef =
        constraints.length > 0 ? firestoreQuery(collectionRef, ...constraints) : collectionRef;
