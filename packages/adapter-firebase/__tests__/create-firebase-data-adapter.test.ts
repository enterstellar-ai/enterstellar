/**
 * @module @enterstellar/adapter-firebase/__tests__/create-firebase-data-adapter
...
vi.mock('firebase/firestore', () => ({
  ...
  onSnapshot: vi.fn().mockImplementation((_ref: unknown, cb: (snapshot: unknown) => void) => {
...
beforeEach(async () => {
  ...
  vi.mocked(firestore.collection)
    .mockClear()
    .mockReturnValue(COLLECTION_REF as unknown as ReturnType<typeof firestore.collection>);
  vi.mocked(firestore.doc)
    .mockClear()
    .mockReturnValue(DOC_REF as unknown as ReturnType<typeof firestore.doc>);
  vi.mocked(firestore.where)
    .mockClear()
    .mockReturnValue(WHERE_CONSTRAINT as unknown as ReturnType<typeof firestore.where>);
  vi.mocked(firestore.query)
    .mockClear()
    .mockReturnValue(QUERY_REF as unknown as ReturnType<typeof firestore.query>);
...
describe('createFirebaseDataAdapter — mutate("update")', () => {
  ...
  it('strips id from the update payload (avoids writing id as a field)', async () => {
    ...
    expect(updateDoc).toHaveBeenCalledWith(DOC_REF, expect.not.objectContaining({ id: 'doc-1' }));
  });
});
...
describe('createFirebaseDataAdapter — subscribe() delegation', () => {
  it('calls onSnapshot() with the collection reference', async () => {
    ...
    expect(collection).toHaveBeenCalledWith(firestore, 'patients');
    expect(onSnapshot).toHaveBeenCalledWith(COLLECTION_REF, expect.any(Function));
  });
