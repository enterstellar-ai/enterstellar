/**
 * @module @enterstellar/devtools/__tests__/components
...
describe('ToggleButton', () => {
  it('renders the ⚡ icon', () => {
    render(<ToggleButton isOpen={false} onToggle={vi.fn()} position="bottom-right" />);
    expect(screen.getByRole('button')).toHaveTextContent('⚡');
  });

  it('fires onToggle when clicked', () => {
    const onToggle = vi.fn();
    render(<ToggleButton isOpen={false} onToggle={onToggle} position="bottom-right" />);
    fireEvent.click(screen.getByRole('button'));
    expect(onToggle).toHaveBeenCalledOnce();
  });

  it('sets aria-expanded to true when open', () => {
    render(<ToggleButton isOpen={true} onToggle={vi.fn()} position="bottom-right" />);
...
describe('StatusBadge', () => {
  ...
  it('sets aria-label with status name', () => {
    render(<StatusBadge status="pass" />);
    expect(screen.getByRole('status')).toHaveAttribute('aria-label', 'Compilation status: pass');
  });

  it('sets data-enterstellar-devtools-status attribute', () => {
    render(<StatusBadge status="fail" />);
    expect(screen.getByRole('status')).toHaveAttribute('data-enterstellar-devtools-status', 'fail');
  });
});
