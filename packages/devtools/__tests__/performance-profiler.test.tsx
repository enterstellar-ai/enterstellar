/**
 * @module @enterstellar/devtools/__tests__/performance-profiler
...
describe('PerformanceProfiler', () => {
  ...
  it('renders empty state when no traces exist', () => {
    renderWithEnterstellar(
      <PerformanceProfiler maxTraces={500} onSelectTrace={vi.fn()} selectedTraceId={null} />,
    );
  ...
  it('handles single-trace edge case without NaN', () => {
    mockStoreTraces = [createTrace('main-1', { totalMs: 42 })];
  ...
  it('renders component names in bar labels', () => {
    mockStoreTraces = [createTrace('main-1', { component: 'PatientVitals', totalMs: 25 })];
  ...
  it('calls onSelectTrace when a bar is clicked', () => {
    ...
    renderWithEnterstellar(
      <PerformanceProfiler maxTraces={500} onSelectTrace={onSelectTrace} selectedTraceId={null} />,
    );
  ...
  it('renders aria-label with latency info on each bar row', () => {
    mockStoreTraces = [createTrace('main-1', { component: 'Card', totalMs: 42, status: 'pass' })];

    renderWithEnterstellar(
      <PerformanceProfiler maxTraces={500} onSelectTrace={vi.fn()} selectedTraceId={null} />,
    );
    expect(screen.getByRole('row')).toHaveAttribute('aria-label', 'Card: 42ms, pass');
  });
