/**
 * @module @enterstellar/devtools/__tests__/use-devtools-traces
...
// ---------------------------------------------------------------------------
// Mock @enterstellar/react — preserves EnterstellarContext for useContext(EnterstellarContext)
// ---------------------------------------------------------------------------
...
describe('useDevtoolsTraces', () => {
  ...
  it('returns empty results when store has no traces', () => {
    const { result } = renderHook(() => useDevtoolsTraces({}, 500), {
  ...
  it('applies filters to buffered traces', () => {
    ...
    const { result } = renderHook(() => useDevtoolsTraces({ status: 'fail' }, 500), {
      wrapper: createTestWrapper(),
    });
  ...
  it('enforces ring buffer max size', () => {
    ...
    const { result } = renderHook(() => useDevtoolsTraces({}, 5), { wrapper: createTestWrapper() });
