/**
 * @module @enterstellar/devtools/__tests__/panels
...
// ---------------------------------------------------------------------------
// Mock @enterstellar/react — preserves EnterstellarContext for useDevtoolsTraces (DT7)
// ---------------------------------------------------------------------------

let mockStoreTraces: ZoneTrace[] = [];

vi.mock('@enterstellar/react', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@enterstellar/react')>();
  return {
    ...actual,
...
describe('TraceTimeline', () => {
  ...
  it('renders empty state when no traces exist', () => {
    renderWithEnterstellar(
      <TraceTimeline maxTraces={500} onSelectTrace={vi.fn()} selectedTraceId={null} />,
    );
    expect(screen.getByText(/no traces yet/i)).toBeDefined();
  });

  it('renders trace count header', () => {
    mockStoreTraces = [createTrace('main-1'), createTrace('main-2')];
  ...
  it('calls onSelectTrace when a row is clicked', () => {
    ...
    renderWithEnterstellar(
      <TraceTimeline maxTraces={500} onSelectTrace={onSelectTrace} selectedTraceId={null} />,
    );
  ...
  it('deselects trace when clicking the same row again', () => {
    ...
    renderWithEnterstellar(
      <TraceTimeline maxTraces={500} onSelectTrace={onSelectTrace} selectedTraceId="main-1" />,
    );
    fireEvent.click(screen.getByRole('row'));
...
describe('ValidationLog', () => {
  ...
  it('renders empty state when no traces exist', () => {
    renderWithEnterstellar(<ValidationLog maxTraces={500} onSelectTrace={vi.fn()} />);
    expect(screen.getByText(/no traces yet/i)).toBeDefined();
  });
  ...
  it('shows singular "issue" for single issue', () => {
    mockStoreTraces = [createTrace('main-1', { status: 'fail' })];
  ...
  it('renders self-correction info for corrected traces', () => {
    mockStoreTraces = [createTrace('main-1', { status: 'corrected' })];
  ...
  it('calls onSelectTrace when entry is clicked', () => {
    ...
    renderWithEnterstellar(<ValidationLog maxTraces={500} onSelectTrace={onSelectTrace} />);
  ...
  it('renders search input with placeholder', () => {
    renderWithEnterstellar(<ValidationLog maxTraces={500} onSelectTrace={vi.fn()} />);
    expect(screen.getByPlaceholderText('Search by component, intent, or zone…')).toBeDefined();
  });
...
describe('PanelStub', () => {
  ...
  it('has accessible aria-label', () => {
    renderWithEnterstellar(<PanelStub tab="cache-dashboard" />);
    expect(screen.getByRole('status')).toHaveAttribute('aria-label', 'Cache — coming soon');
  });
});
