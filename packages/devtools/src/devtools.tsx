...
/**
 * @module @enterstellar/devtools/enterstellar-devtools
...
type EnterstellarDevToolsProps = {
   ...
   * including the real `RenderCache` from `@enterstellar/cache` (L5).
   *
...
export function EnterstellarDevTools(props: EnterstellarDevToolsProps): React.JSX.Element | null {
   ...
   */
  const selectedTraceId = useMemo((): string | null => selectedTrace?.id ?? null, [selectedTrace]);
  ...
  return (
    <>
      ...
      <ToggleButton isOpen={isOpen} onToggle={handleToggle} position={position} />
      ...
      {isOpen && (
        ...
        >
          ...
          <div style={panelStyles['header']}>
            <span style={panelStyles['headerTitle']}>⚡ Enterstellar DevTools</span>
            <button
              type="button"
          ...
          {/* ─── Tab Bar ──────────────────────────────── */}
          <div style={tabBarStyles['container']} role="tablist" aria-label="DevTools tabs">
            {ALL_TABS.map((tab) => {
          ...
          {/* ─── Tab Content ──────────────────────────── */}
          <div style={panelStyles['content']} role="tabpanel" aria-label={TAB_LABELS[activeTab]}>
            ...
            {activeTab === 'component-inspector' && (
              <ComponentInspector selectedTrace={selectedTrace} />
            )}

            {activeTab === 'validation-log' && (
              <ValidationLog maxTraces={maxTraces} onSelectTrace={handleSelectTrace} />
            )}

            {activeTab === 'cache-dashboard' && <CacheDashboard cache={cache ?? null} />}
            ...
            {activeTab === 'replay-mode' && <ReplayMode selectedTrace={selectedTrace} />}

            {DEFERRED_TABS.has(activeTab) && <PanelStub tab={activeTab} />}
          </div>
        </div>
      )}
    </>
