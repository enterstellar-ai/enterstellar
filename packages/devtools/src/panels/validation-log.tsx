...
/**
 * @module @enterstellar/devtools/panels/validation-log
...
export function ValidationLog(props: ValidationLogProps): React.JSX.Element {
  ...
  const issueTraces = useMemo((): readonly ZoneTrace[] => {
    const issues = allTraces.filter(
      (trace) => trace.compilation.status === 'fail' || trace.compilation.status === 'corrected',
    );
  ...
  return (
    <div style={sharedPanelStyles['panelRoot']} data-enterstellar-devtools-panel="validation-log">
      {/* Header */}
      ...
      {/* Search Bar */}
      <div style={sharedPanelStyles['searchBar']}>
        <input
          type="text"
      ...
      {/* Issue List */}
      <div style={{ flex: 1, overflow: 'auto' }} role="log" aria-label="Validation issues">
        {issueTraces.length === 0 ? (
          ...
          issueTraces.map((trace) => (
            <ValidationEntry key={trace.id} trace={trace} onClick={handleEntryClick} />
          ))
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={sharedPanelStyles['componentName']}>{trace.intent.component}</span>
      </div>

      {/* Error Details */}
      <div style={validationLogStyles['message']}>
        {trace.compilation.errors.length}{' '}
        {trace.compilation.errors.length === 1 ? 'error' : 'errors'}
      ...
      {trace.compilation.status === 'corrected' && (
        <div style={validationLogStyles['fix']}>✓ Self-corrected successfully</div>
      )}
