/**
 * @module @enterstellar/telemetry/queue/indexed-db-queue
 ...
 * (`enterstellar-telemetry`) that is completely isolated from `@enterstellar/state`'s
 * `enterstellar-store` database (TL4).
 *
 * **No `idb-keyval` dependency.** This module wraps the raw IndexedDB
 * API in a thin utility. `idb-keyval` is reserved for `@enterstellar/state` (S6).
 *
...
function openDatabase(): Promise<IDBDatabase> {
  return new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () => {
      reject(
        new Error(
          `[@enterstellar/telemetry] Failed to open IndexedDB "${DB_NAME}": ${String(request.error?.message)}`,
        ),
      );
    };
  });
...
function promisifyRequest<T>(request: IDBRequest<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    request.onerror = () => {
      reject(request.error ?? new Error('[@enterstellar/telemetry] IndexedDB request failed.'));
    };
  });
}
...
function promisifyTransaction(tx: IDBTransaction): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    tx.onerror = () => {
      reject(tx.error ?? new Error('[@enterstellar/telemetry] IndexedDB transaction failed.'));
    };
    tx.onabort = () => {
      reject(tx.error ?? new Error('[@enterstellar/telemetry] IndexedDB transaction aborted.'));
    };
  });
}
 ...
 * The database is completely isolated from `@enterstellar/state`'s `enterstellar-store`.
 *
    ...
    async dequeue(count: number): Promise<readonly ForgeSignal[]> {
      ...
      return new Promise<readonly ForgeSignal[]>((resolve, reject) => {
        ...
        cursorRequest.onerror = () => {
          reject(
            cursorRequest.error ??
              new Error('[@enterstellar/telemetry] IndexedDB cursor request failed.'),
          );
        };
      });
    },
