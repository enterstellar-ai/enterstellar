...
export function registerEducationRenderers(): void {
    ...
    return (
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <div className="flex items-start justify-between mb-3">
          ...
          <span className="text-lg font-bold text-[var(--token-accent,oklch(0.65_0.15_250))] tabular-nums">
            {completion}%
          </span>
        </div>
        ...
        <div className="space-y-1.5">
          {modules.map((mod, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              ...
              <span
                className={`flex-1 ${str(mod['status']) === 'completed' ? 'text-success' : str(mod['status']) === 'locked' ? 'text-playground-muted' : 'text-[var(--token-text-primary,theme(colors.neutral-200))]'}`}
              >
                {str(mod['name'])}
              </span>
              <span className="text-[10px] text-playground-muted tabular-nums">
                {num(mod['durationMinutes'])}min
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-2 text-center">
            ...
            <p className="text-lg font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
              {num(metrics['gpa']).toFixed(1)}
            </p>
          </div>
          <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-2 text-center">
            <p className="text-[10px] uppercase tracking-wider text-playground-muted">Engagement</p>
            <p className="text-lg font-bold text-success tabular-nums">
              {num(metrics['engagementScore'])}%
            </p>
          </div>
          <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-2 text-center">
            <p className="text-[10px] uppercase tracking-wider text-playground-muted">Streak</p>
            <p className="text-lg font-bold text-[var(--token-accent,oklch(0.65_0.15_250))] tabular-nums">
              {num(metrics['currentStreak'])}d
            </p>
          </div>
        </div>
          ...
          <div className="pt-3 border-t border-[var(--token-card-border,theme(colors.playground-border))] space-y-1">
            {recentGrades.map((grade, i) => (
              <div key={i} className="flex items-center justify-between text-xs">
                <span className="text-[var(--token-text-primary,theme(colors.neutral-200))] truncate">
                  {str(grade['course'])}
                </span>
    ...
    return (
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <div className="flex items-start justify-between mb-3">
          ...
          <span
            className={`text-xs font-bold px-2.5 py-1 rounded-full ${passed ? 'bg-success/20 text-success' : 'bg-error/20 text-error'}`}
          >
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="flex-1">
            <div className="h-3 rounded-full bg-[var(--token-surface,theme(colors.playground-panel))] overflow-hidden relative">
              ...
              <div
                className="absolute top-0 h-full w-px bg-[var(--token-text-primary,theme(colors.neutral-100))]/30"
                style={{ left: `${String(passingScore)}%` }}
              />
            </div>
          </div>
          <span className="text-xl font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
            {score}%
          </span>
        </div>

        <p className="text-[10px] text-playground-muted mb-3">
          ⏱ {timeTaken} · Passing: {passingScore}%
        </p>
        ...
        {breakdown.length > 0 && (
          <div className="pt-3 border-t border-[var(--token-card-border,theme(colors.playground-border))] space-y-1.5">
            {breakdown.map((section, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between text-xs">
                  <span className="text-[var(--token-text-primary,theme(colors.neutral-200))]">
                    {str(section['section'])}
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="text-playground-muted tabular-nums">
                      {correct}/{total}
                    </span>
                    <span
                      className={`font-semibold tabular-nums ${pct >= 80 ? 'text-success' : pct >= 60 ? 'text-warning' : 'text-error'}`}
                    >
                      {pct}%
                    </span>
                  </div>
  ...
  registerRenderer('CurriculumMap', (props: Record<string, unknown>): JSX.Element => {
    ...
    const masteryBadge: Record<string, { cls: string; label: string }> = {
      ...
      novice: {
        cls: 'bg-[var(--token-accent,oklch(0.65_0.15_250))]/20 text-[var(--token-accent,oklch(0.65_0.15_250))]',
        label: 'Novice',
      },
        ...
        <div className="space-y-0">
          {topics.map((topic, i) => {
            ...
            const badge = masteryBadge[mastery] ??
              masteryBadge['not-started'] ?? { cls: '', label: mastery };
            return (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <span
                    className={`size-3 rounded-full border-2 shrink-0 ${unlocked ? (completedAt !== '' ? 'bg-success border-success' : 'bg-[var(--token-accent,oklch(0.65_0.15_250))] border-[var(--token-accent,oklch(0.65_0.15_250))]') : 'bg-transparent border-playground-muted'}`}
                  />
                  {i < topics.length - 1 && (
                    <div
                      className={`w-px flex-1 min-h-[16px] ${unlocked ? 'bg-success/50' : 'bg-playground-muted/30'}`}
                    />
                  )}
                </div>
                <div className="pb-2 min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xs ${unlocked ? 'text-[var(--token-text-primary,theme(colors.neutral-200))]' : 'text-playground-muted'}`}
                    >
                      {str(topic['name'])}
                    </span>
                    <span
                      className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${badge.cls}`}
                    >
                      {badge.label}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] text-playground-muted">
                    ...
                    {completedAt !== '' && (
                      <span className="text-success">Completed {completedAt}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          {grid.map((dayRow, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-1.5">
                <span className="text-[9px] text-playground-muted w-6 shrink-0 text-right">
                  {day}
                </span>
        ...
        <div className="grid grid-cols-4 gap-2 pt-2 border-t border-[var(--token-card-border,theme(colors.playground-border))]">
          <div className="text-center">
            <p className="text-sm font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
              {num(metrics['totalSessions'])}
            </p>
            <p className="text-[8px] text-playground-muted uppercase">Sessions</p>
          </div>
          <div className="text-center">
            <p className="text-sm font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
              {num(metrics['averageDurationMinutes']).toFixed(0)}m
            </p>
            <p className="text-[8px] text-playground-muted uppercase">Avg Duration</p>
          </div>
          <div className="text-center">
            <p className="text-sm font-bold text-[var(--token-accent,oklch(0.65_0.15_250))]">
              {str(metrics['peakHour'])}
            </p>
        ...
        <div className="divide-y divide-[var(--token-card-border,theme(colors.playground-border))]">
          {certifications.map((cert, i) => {
            ...
            const badge = statusBadge[status] ??
              statusBadge['not-started'] ?? { cls: '', label: status };
            ...
            return (
              <div key={i} className="px-5 py-3">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[var(--token-text-primary,theme(colors.neutral-200))] truncate">
                      {str(cert['name'])}
                    </p>
                    <p className="text-[10px] text-playground-muted">{str(cert['issuer'])}</p>
                  </div>
                  <span
                    className={`text-[9px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${badge.cls}`}
                  >
                    {badge.label}
                  </span>
                </div>
                ...
                <div className="space-y-0.5 mb-1.5">
                  {requirements.map((req, j) => (
                    <div key={j} className="flex items-center gap-1.5 text-[10px]">
                      <span
                        className={
                          bool(req['completed']) ? 'text-success' : 'text-playground-muted'
                        }
                      >
                        {bool(req['completed']) ? '\u2713' : '\u25CB'}
                      </span>
                      <span
                        className={
                          bool(req['completed'])
                            ? 'text-playground-muted line-through'
                            : 'text-[var(--token-text-primary,theme(colors.neutral-200))]'
                        }
                      >
                        {str(req['description'])}
                      </span>
                  ...
                  <div className="mt-1.5">
                    <div className="flex items-center justify-between text-[10px] mb-0.5">
                      ...
                      <span className="text-[var(--token-text-primary,theme(colors.neutral-200))] tabular-nums">
                        {num(ceu['earned'])}/{num(ceu['required'])}
                      </span>
                    </div>
                    <div className="h-1 rounded-full bg-[var(--token-surface,theme(colors.playground-panel))]">
                      <div
                        className="h-full rounded-full bg-[var(--token-accent,oklch(0.65_0.15_250))] transition-all"
                        style={{
                          width: `${String(num(ceu['required']) > 0 ? Math.min((num(ceu['earned']) / num(ceu['required'])) * 100, 100) : 0)}%`,
                        }}
                      />
                    </div>
                  </div>
