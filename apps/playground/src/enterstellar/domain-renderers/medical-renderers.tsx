...
export function registerMedicalRenderers(): void {
    ...
    return (
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-mono text-playground-muted bg-[var(--token-surface,theme(colors.playground-panel))] px-2 py-0.5 rounded">
            {patientId}
          </span>
        </div>
          ...
          {events.map((event, i) => (
            <div key={i} className="flex gap-3">
              ...
              <div className="pb-4 min-w-0">
                <p className="text-sm font-medium text-[var(--token-text-primary,theme(colors.neutral-200))]">
                  {str(event['title'])}
                </p>
                <p className="text-[10px] text-[var(--token-text-secondary,theme(colors.playground-muted))]">
                  {str(event['provider'])} · {str(event['date'])}
                </p>
                {str(event['details']) !== '' && (
                  <p className="text-xs text-[var(--token-text-secondary,theme(colors.neutral-400))] mt-1">
                    {str(event['details'])}
                  </p>
                )}
              </div>
  ...
  registerRenderer('VitalsMonitor', (props: Record<string, unknown>): JSX.Element => {
    ...
    function VitalCard({
      ...
      return (
        <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-3 text-center">
          <p className="text-[10px] uppercase tracking-wider text-[var(--token-text-secondary,theme(colors.playground-muted))] mb-1">
            {label}
          </p>
          <p
            className={`text-xl font-bold tabular-nums ${statusColor[status] ?? 'text-[var(--token-text-primary,theme(colors.neutral-100))]'}`}
          >
            {value}
          </p>
    ...
    return (
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <h3 className="text-sm font-semibold text-[var(--token-text-primary,theme(colors.neutral-100))] mb-3">
          {patientName} — Vitals
        </h3>
        <div className="grid grid-cols-2 gap-2">
          <VitalCard
            label="Heart Rate"
            value={String(num(hr['value']))}
            unit={str(hr['unit'], 'bpm')}
            status={str(hr['status'])}
          />
          <VitalCard
            label="Blood Pressure"
            value={`${String(num(bp['systolic']))}/${String(num(bp['diastolic']))}`}
            unit="mmHg"
            status={str(bp['status'])}
          />
          <VitalCard
            label="SpO₂"
            value={`${String(num(spo2['value']))}%`}
            unit="saturation"
            status={str(spo2['status'])}
          />
          <VitalCard
            label="Temperature"
            value={num(temp['value']).toFixed(1)}
            unit={str(temp['unit'], '°F')}
  ...
  registerRenderer('ClinicalAlert', (props: Record<string, unknown>): JSX.Element => {
    ...
    const style = severityStyles[severity] ??
      severityStyles['info'] ?? { border: 'border-primary-500/30', bg: 'bg-primary-500/10' };
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[9px] uppercase tracking-wider text-playground-muted">
                {category.replace('-', ' ')}
              </span>
            </div>
  ...
  registerRenderer('MedicationSchedule', (props: Record<string, unknown>): JSX.Element => {
    ...
    const routeBadge: Record<string, string> = {
      oral: 'bg-primary-500/20 text-primary-400',
      iv: 'bg-error/20 text-error',
      im: 'bg-warning/20 text-warning',
      sc: 'bg-success/20 text-success',
      topical: 'bg-playground-muted/20 text-playground-muted',
      inhaled: 'bg-accent/20 text-accent',
    ...
    const statusColor: Record<string, string> = {
      active: 'text-success',
      held: 'text-warning',
      discontinued: 'text-playground-muted',
      prn: 'text-accent',
    };
              ...
              <div key={i} className="px-5 py-3">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={`text-xs ${statusColor[status] ?? 'text-playground-muted'}`}>
                      {statusIcon[status] ?? '\u25CF'}
                    </span>
                    <span className="text-sm font-medium text-[var(--token-text-primary,theme(colors.neutral-200))] truncate">
                      {str(med['name'])}
                    </span>
                    <span
                      className={`text-[9px] font-semibold uppercase px-1.5 py-0.5 rounded ${routeBadge[route] ?? 'bg-playground-muted/20 text-playground-muted'}`}
                    >
                      {route}
                    </span>
                  </div>
                  <span className="text-xs text-[var(--token-text-primary,theme(colors.neutral-100))] font-bold tabular-nums shrink-0">
                    {str(med['dosage'])}
                  </span>
                </div>
                <p className="text-[10px] text-playground-muted">
                  {str(med['frequency'])}
                  {str(med['nextDue']) !== '' ? ` \u00B7 Next: ${str(med['nextDue'])}` : ''}
                </p>
                {conflicts.length > 0 &&
                  conflicts.map((c, j) => (
                    <div
                      key={j}
                      className="mt-1.5 flex items-start gap-1.5 text-[10px] text-error bg-error/10 px-2 py-1 rounded"
                    >
                      <span className="shrink-0">\u26A0</span>
                      <span>
                        <b>{str(c['severity']).toUpperCase()}</b> interaction with{' '}
                        {str(c['withDrug'])}: {str(c['description'])}
                      </span>
                    </div>
                  ))}
              </div>
      ...
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] overflow-hidden">
        ...
        <div className="divide-y divide-[var(--token-card-border,theme(colors.playground-border))]">
          {results.map((result, i) => {
            ...
            const prevVal =
              result['previousValue'] !== undefined ? num(result['previousValue']) : undefined;
            ...
            return (
              <div key={i} className="px-5 py-2 flex items-center justify-between">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-xs text-[var(--token-text-primary,theme(colors.neutral-200))] w-24 truncate">
                    {str(result['testName'])}
                  </span>
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`text-sm font-bold tabular-nums ${flag !== 'normal' ? 'text-[var(--token-text-primary,theme(colors.neutral-100))]' : 'text-success'}`}
                  >
                    {value}
                  </span>
                  <span className="text-[10px] text-playground-muted tabular-nums w-16 text-right">
                    {refLow}&ndash;{refHigh}
                  </span>
                  <span className="text-[10px] text-playground-muted w-6">
                    {str(result['unit'])}
                  </span>
                  {delta !== undefined && (
                    <span
                      className={`text-[10px] tabular-nums ${delta > 0 ? 'text-error' : delta < 0 ? 'text-success' : 'text-playground-muted'}`}
                    >
            ...
            return (
              <div key={i} className="px-5 py-2.5 flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <span
                    className={`size-2 rounded-full ${shiftDot[shift] ?? 'bg-playground-muted'} shrink-0`}
                  />
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-[var(--token-text-primary,theme(colors.neutral-200))] truncate">
                        {str(member['name'])}
                      </span>
                      {designation !== '' && (
                        <span className="text-[8px] font-bold uppercase px-1 py-0.5 rounded bg-[var(--token-accent,oklch(0.65_0.15_250))]/15 text-[var(--token-accent,oklch(0.65_0.15_250))]">
                          {designation}
                        </span>
                      )}
                    </div>
                    <p className="text-[10px] text-playground-muted">
                      {str(member['contact'])} &middot; Since {str(member['since'])}
                    </p>
                  </div>
                </div>
                <span className="text-[10px] font-semibold text-playground-muted uppercase shrink-0">
                  {roleLabel[role] ?? role}
                </span>
              </div>
            );
