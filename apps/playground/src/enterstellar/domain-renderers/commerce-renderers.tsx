        ...
        <div className="divide-y divide-[var(--token-card-border,theme(colors.playground-border))]">
          {products.map((product, i) => {
            ...
            const badge = stockBadge[status] ??
              stockBadge['in-stock'] ?? {
                label: 'Unknown',
                cls: 'bg-playground-muted/20 text-playground-muted',
              };
            return (
              <div
                key={i}
                className="flex items-center justify-between px-5 py-3 hover:bg-[var(--token-surface,theme(colors.playground-panel))] transition-colors"
              >
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-[var(--token-text-secondary,theme(colors.playground-muted))]">
                    {str(product['sku'])} · {str(product['category'])}
                  </p>
                </div>
          ...
          {stages.map((stage, i) => {
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-xs font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
                  {count}
                </span>
                <div
                ...
                />
                <span className="text-[9px] text-[var(--token-text-secondary,theme(colors.playground-muted))] text-center leading-tight">
                  {str(stage['name'])}
                </span>
              </div>
            );
                ...
                <div className="flex items-center justify-between mb-1">
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-playground-muted">
                      {str(item['sku'])} · {num(item['dailyVelocity']).toFixed(1)}/day
                    </p>
                  </div>
                  <span
                    className={`text-[10px] font-semibold ${statusColor[status] ?? 'text-playground-muted'}`}
                  >
      ...
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[10px] text-playground-muted">
              {cohortSize.toLocaleString()} customers &middot; {channel}
            </p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-success tabular-nums">
              {num(metrics['retentionRate'])}%
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-playground-muted">Churn Risk</span>
            <span
              className={`text-xs font-bold tabular-nums ${num(metrics['churnRisk']) > 30 ? 'text-error' : num(metrics['churnRisk']) > 15 ? 'text-warning' : 'text-success'}`}
            >
              {num(metrics['churnRisk'])}%
            </span>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        ...
        <p className="text-[10px] text-playground-muted mb-3">
          {orderId} &middot; {carrier} &middot; {trackingNumber}
        </p>
            ...
            return (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <span
                    className={`size-3 rounded-full border-2 shrink-0 ${completed ? 'bg-success border-success' : 'bg-transparent border-playground-muted'}`}
                  />
                  {i < milestones.length - 1 && (
                    <div
                      className={`w-px flex-1 min-h-[16px] ${completed ? 'bg-success' : 'bg-playground-muted/30'}`}
                    />
                  )}
                </div>
                <div className="pb-2 min-w-0">
                  <p
                    className={`text-xs ${completed ? 'text-[var(--token-text-primary,theme(colors.neutral-200))]' : 'text-playground-muted'}`}
                  >
        ...
        <div className="pt-2 border-t border-[var(--token-card-border,theme(colors.playground-border))] flex items-center justify-between">
          ...
          {currentLoc !== '' && (
            <span className="text-[10px] text-[var(--token-text-secondary,theme(colors.playground-muted))]">
              Currently: {currentLoc}
            </span>
          )}
        </div>
      </div>
              ...
              {reasons.map((r, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2">
                    <span className="text-[10px] text-[var(--token-text-primary,theme(colors.neutral-200))] w-28 truncate">
                      {str(r['reason']).replace(/-/g, ' ')}
                    </span>
                    <div className="flex-1 h-1.5 rounded-full bg-[var(--token-surface,theme(colors.playground-panel))]">
                      <div
                        className="h-full rounded-full bg-error/60 transition-all"
                        style={{ width: `${String(pct)}%` }}
                      />
                    </div>
                    <span className="text-[10px] text-playground-muted tabular-nums w-8 text-right">
                      {pct}%
                    </span>
                  </div>
                );
          ...
          <div className="pt-2 border-t border-[var(--token-card-border,theme(colors.playground-border))]">
            ...
            <div className="flex gap-2">
              {resolutions.map((r, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-1.5 text-center"
                >
                  <p className="text-xs font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
                    {num(r['percentage'])}%
                  </p>
                  <p className="text-[8px] text-playground-muted leading-tight">
                    {str(r['type']).replace(/-/g, ' ')}
                  </p>
                </div>
              ))}
            </div>
