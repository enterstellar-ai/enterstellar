...
export function registerSaasRenderers(): void {
        ...
        <div className="flex gap-2 overflow-x-auto">
          {stages.map((stage, i) => {
            ...
            return (
              <div
                key={i}
                className="flex-1 min-w-[100px] rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-3"
              >
                <p className="text-[10px] uppercase tracking-wider text-[var(--token-text-secondary,theme(colors.playground-muted))] mb-2 truncate">
                  {str(stage['name'])}
                </p>
                <p className="text-lg font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
                  {dealCount}
                </p>
                <p className="text-[10px] text-[var(--token-text-secondary,theme(colors.playground-muted))] tabular-nums">
      ...
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        <div className="flex items-start justify-between mb-3">
          <div className="min-w-0">
            <h3 className="text-base font-semibold text-[var(--token-text-primary,theme(colors.neutral-100))] truncate">
              {company}
            </h3>
          ...
          {priority !== '' && (
            <span
              className={`text-[9px] font-semibold px-2 py-0.5 rounded-full uppercase ${priorityBadge[priority] ?? ''}`}
            >
              {priority}
            </span>
          )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-playground-muted mb-0.5">
              Value
            </p>
            <p className="text-lg font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
              {formatted}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[var(--token-card-border,theme(colors.playground-border))]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--token-text-secondary,theme(colors.playground-muted))]">
              {assignedTo}
            </span>
          </div>
          ...
          {activities.map((activity, i) => (
            <div key={i} className="flex gap-3">
              ...
              <div className="pb-3 min-w-0">
                ...
                <p className="text-[10px] text-[var(--token-text-secondary,theme(colors.playground-muted))]">
                  {str(activity['rep'])} → {str(activity['contact'])} · {str(activity['timestamp'])}
                </p>
  ...
  registerRenderer('ForecastGauge', (props: Record<string, unknown>): JSX.Element => {
    ...
    const attainColor =
      attainment >= 100 ? 'text-success' : attainment >= 70 ? 'text-warning' : 'text-error';
      ...
      <div className="rounded-xl border border-[var(--token-card-border,theme(colors.playground-border))] bg-[var(--token-card-bg,theme(colors.playground-surface))] p-5">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-2 text-center">
            ...
            <p className="text-sm font-bold text-[var(--token-accent,oklch(0.65_0.15_250))] tabular-nums">
              {fmt.format(weighted)}
            </p>
          </div>
          <div className="rounded-lg bg-[var(--token-surface,theme(colors.playground-panel))] p-2 text-center">
            <p className="text-[9px] uppercase tracking-wider text-playground-muted">Best/Worst</p>
            <p className="text-[10px] font-bold text-[var(--token-text-primary,theme(colors.neutral-200))] tabular-nums">
              {fmt.format(worstCase)}&ndash;{fmt.format(bestCase)}
            </p>
          </div>
        </div>
        ...
        {stageConf.length > 0 && (
          <div className="pt-2 border-t border-[var(--token-card-border,theme(colors.playground-border))]">
            <p className="text-[10px] text-playground-muted uppercase tracking-wider mb-1.5">
              Confidence Breakdown
            </p>
            {stageConf.map((sc, i) => (
              <div key={i} className="flex items-center justify-between py-0.5">
                <span className="text-[10px] text-[var(--token-text-primary,theme(colors.neutral-200))]">
                  {str(sc['stage'])}
                </span>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-[var(--token-text-primary,theme(colors.neutral-100))] tabular-nums">
                    {fmt.format(num(sc['weighted']))}
                  </span>
                </div>
              </div>
            ))}
        ...
        <div className="mb-2">
          ...
          {signals.map((sig, j) => (
            <div key={j} className="flex items-center justify-between py-0.5">
              <span className="text-[10px] text-[var(--token-text-primary,theme(colors.neutral-200))] truncate">
                {str(sig['signal'])}
              </span>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-playground-muted truncate max-w-[80px]">
                  {str(sig['value'])}
                </span>
                <span className="text-[10px] font-bold tabular-nums text-[var(--token-text-primary,theme(colors.neutral-100))]">
                  {num(sig['score'])}
                </span>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[10px] text-playground-muted">
              {company} &middot; Last: {lastActivity}
            </p>
          </div>
          <div className="text-center">
            <div className="text-2xl font-black tabular-nums text-[var(--token-text-primary,theme(colors.neutral-100))]">
              {overallScore}
            </div>
            <span className={`text-sm font-bold ${gradeColor[grade] ?? 'text-playground-muted'}`}>
              Grade {grade}
            </span>
          </div>
        </div>
  ...
  registerRenderer('IntegrationStatus', (props: Record<string, unknown>): JSX.Element => {
    ...
    const statusDot: Record<string, string> = {
      synced: 'bg-success',
      syncing: 'bg-[var(--token-accent,oklch(0.65_0.15_250))]',
      error: 'bg-error',
      paused: 'bg-warning',
      disconnected: 'bg-playground-muted',
    };
              ...
              <div key={i} className={`px-5 py-3 ${status === 'error' ? 'bg-error/5' : ''}`}>
                <div className="flex items-center justify-between mb-0.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`size-2 rounded-full shrink-0 ${statusDot[status] ?? 'bg-playground-muted'} ${status === 'syncing' ? 'animate-pulse' : ''}`}
                    />
                ...
                <div className="flex items-center justify-between text-[10px] text-playground-muted">
                  <span>
                    {str(integ['provider'])} &middot; {num(integ['recordsSynced']).toLocaleString()}{' '}
                    records
                  </span>
                  <span className={errorRate > 1 ? 'text-error' : ''}>Err: {errorRate}%</span>
                </div>
                {errorMsg !== '' && (
                  <p className="text-[10px] text-error mt-1 bg-error/10 px-2 py-0.5 rounded">
                    {errorMsg}
                  </p>
                )}
              </div>
